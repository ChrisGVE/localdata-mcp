...
class VersionComparison:
    ...
    def simulate_v130_batch_processing(
        ...
        for i in range(iterations):
            ...
            try:
                ...
                self.logger.debug(
                    f"v1.3.0 iteration {i+1}: {rows_processed} rows in {execution_time:.3f}s"
                )
    ...
    def test_v131_streaming_processing(
        self, connection_string: str, query: str, dataset_size: str, iterations: int = 3
    ) -> BenchmarkResult:
        ...
        for i in range(iterations):
            ...
            try:
                ...
                self.logger.debug(
                    f"v1.3.1 iteration {i+1}: {rows_processed} rows in {execution_time:.3f}s, "
    ...
    def compare_versions(
        self, dataset_rows: List[int] = None, iterations: int = 3
    ) -> Dict[str, Any]:
        ...
        results = {
            ...
            "test_config": {"dataset_rows": dataset_rows, "iterations": iterations},
            "results": [],
            "summary": {},
        }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for rows in dataset_rows:
                ...
                connection_string = DatasetGenerator.create_test_database(
                    str(db_path),
                    rows,
                    text_heavy=True,  # Use text-heavy data for realistic comparison
                )
                ...
                comparison = self._calculate_improvements(
                    v130_result, v131_result, rows
                )

                results["results"].append(comparison)

                self.logger.info(
                    f"Completed {rows} rows: "
    ...
    def _calculate_improvements(
        self, v130_result: BenchmarkResult, v131_result: BenchmarkResult, rows: int
    ) -> Dict[str, Any]:
        ...
        execution_improvement = (
            (v130_result.execution_time_seconds - v131_result.execution_time_seconds)
            / v130_result.execution_time_seconds
            * 100
        )

        memory_improvement = (
            (v130_result.peak_memory_mb - v131_result.peak_memory_mb)
            / v130_result.peak_memory_mb
            * 100
        )

        rate_improvement = (
            (
                v131_result.processing_rate_rows_per_second
                - v130_result.processing_rate_rows_per_second
            )
            / v130_result.processing_rate_rows_per_second
            * 100
        )
        ...
        return {
            ...
            "improvement_summary": self._classify_improvement(
                execution_improvement, memory_improvement
            ),
        }

    def _classify_improvement(
        self, exec_improvement: float, memory_improvement: float
    ) -> str:
    ...
    def _generate_comparison_summary(
        ...
        exec_improvements = [
            r["execution_time_improvement"] for r in successful_results
        ]
        memory_improvements = [r["memory_improvement"] for r in successful_results]
        rate_improvements = [
            r["processing_rate_improvement"] for r in successful_results
        ]

        return {
            "total_tests": len(results),
            "successful_tests": len(successful_results),
            "average_execution_improvement": sum(exec_improvements)
            / len(exec_improvements),
            "average_memory_improvement": sum(memory_improvements)
    ...
    def _write_comparison_report(self, file, results: Dict[str, Any]):
        ...
        file.write(
            f"- **Average Execution Time Improvement:** {summary['average_execution_improvement']:.1f}%\n"
        )
        file.write(
            f"- **Average Memory Usage Improvement:** {summary['average_memory_improvement']:.1f}%\n"
        )
        file.write(
            f"- **Average Processing Rate Improvement:** {summary['average_rate_improvement']:.1f}%\n"
        )
        file.write(
            f"- **Tests with Consistent Improvement:** {summary['successful_tests']}/{summary['total_tests']}\n\n"
        )

        file.write("## Detailed Results\n\n")
        file.write(
            "| Dataset Size | Execution Improvement | Memory Improvement | Rate Improvement | Summary |\n"
        )
        file.write(
            "|-------------|---------------------|-------------------|------------------|----------|\n"
        )
        ...
        if summary["average_execution_improvement"] > 10:
            file.write(
                "✅ **Significant speed improvements** - v1.3.1 streaming architecture is measurably faster\n\n"
            )

        if summary["average_memory_improvement"] > 10:
            file.write(
                "✅ **Significant memory efficiency gains** - Streaming reduces peak memory usage\n\n"
            )

        if summary["consistent_improvement"]:
            file.write(
                "✅ **Consistent performance gains** - Improvements across all dataset sizes\n\n"
            )
        else:
            file.write(
                "⚠️ **Mixed performance results** - Some scenarios show regressions\n\n"
            )

        file.write("## Recommendations\n\n")

        if (
            summary["average_execution_improvement"] > 5
            and summary["average_memory_improvement"] > 5
        ):
            file.write(
                "- ✅ **Recommended:** Deploy v1.3.1 streaming architecture for production use\n"
            )
            file.write(
                "- 📈 **Benefit:** Users will experience faster queries with lower memory usage\n"
            )
        elif summary["average_execution_improvement"] > 0:
            file.write(
                "- ⚠️ **Consider:** v1.3.1 shows improvements but may need optimization for specific use cases\n"
            )
        else:
            file.write(
                "- 🚨 **Investigate:** Performance regressions detected - review implementation\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("version_comparison"),
    ...
    try:
        ...
        print(
            f"Average Execution Improvement: {summary.get('average_execution_improvement', 0):.1f}%"
        )
        print(
            f"Average Memory Improvement: {summary.get('average_memory_improvement', 0):.1f}%"
        )
        print(
