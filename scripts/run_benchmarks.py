...
Examples:
  ...
  python run_benchmarks.py --quick
    ...
    parser.add_argument(
        "--category",
        choices=["all", "token", "streaming", "memory", "regression"],
        default="all",
        help="Category of benchmarks to run (default: all)",
    )
    ...
    parser.add_argument(
        "--no-detailed", action="store_true", help="Skip saving detailed metrics"
    )
...
def print_summary_table(results: dict):
    ...
    if results["success_rate"] > 0:
        print(
            f"Average Processing Rate: {results.get('average_processing_rate', 0):.0f} rows/second"
        )
        print(
            f"Average Memory Usage: {results.get('average_memory_usage_mb', 0):.1f} MB"
        )
...
def main():
    ...
    if not args.quiet:
        ...
        logger.info(
            f"Configuration: Category={args.category}, Iterations={args.iterations}"
        )
    ...
    try:
        ...
        if results["success_rate"] < 0.8:  # Less than 80% success rate
            ...
            sys.exit(1)
        elif results.get("performance_regressions", 0) > results.get(
