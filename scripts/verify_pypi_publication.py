...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        results.append(
            run_command(
                "pip index versions localdata-mcp", "Check package availability on PyPI"
            )
        )

        # 2. Create virtual environment
        results.append(
            run_command("python3 -m venv test_env", "Create test virtual environment")
        )
        ...
        results.append(
            run_command(
                "test_env/bin/localdata-mcp --help | head -10", "Test command-line tool"
            )
        )
        ...
        results.append(
            run_command("test_env/bin/pip check", "Check dependency conflicts")
        )
