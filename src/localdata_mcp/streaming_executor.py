...
class StreamingQueryExecutor:
    ...
    def __init__(self, config: Optional[PerformanceConfig] = None):
        ...
        with logging_manager.context(
            operation="streaming_executor_init", component="streaming_executor"
        ):
            logger.info(
    ...
    def execute_streaming(
        ...
        request_id = logging_manager.log_query_start(
            database_name or "file_source", f"streaming_query_{query_id}", "streaming"
        )
        ...
        try:
            if timeout_config:
                with timeout_manager.timeout_context(
                    operation_id, timeout_config, cleanup_on_timeout
                ) as context:
            ...
            logging_manager.log_performance_metrics(
                "streaming_executor",
                {
                    ...
                    "memory_usage_bytes": (
                        memory_status.total_gb - final_memory.available_gb
                    )
                    * 1024**3,
                    "chunk_count": metadata.get("chunk_count", 0),
                    "total_rows": len(result) if result is not None else 0,
                    "memory_efficiency": final_memory.available_gb
                    / memory_status.total_gb,
                },
            )
            ...
            # Attach structured error classification for callers
            db_type = self._detect_db_type(source)
            e.structured_error = classify_error(e, db_type)  # type: ignore[attr-defined]
            raise

    def _execute_streaming_with_timeout(
        self,
        source: StreamingDataSource,
        query_id: str,
        chunk_size: int,
        ...
        buffer_memory_limit = min(
            self.config.memory_limit_mb // 4, 500
        )  # Use 1/4 of limit, max 500MB
        result_buffer = ResultBuffer(
            query_id, "streaming", "streaming_query", buffer_memory_limit
        )
        ...
        try:
            ...
            for chunk in chunk_iterator:
                ...
                chunk_memory = processed_chunk.memory_usage(deep=True).sum() / (
                    1024 * 1024
                )
                ...
                if (
                    ...
                    logger.info(
                        f"Stopping initial streaming due to memory constraints after {chunk_number} chunks"
                    )
                    break
        ...
        if first_chunk is not None and len(first_chunk) > 0:
            metadata.update(
                self._generate_token_metadata(first_chunk, total_rows_processed)
            )

        logger.info(
            f"Streaming execution completed: {total_rows_processed} rows in {execution_time:.3f}s, "
            f"{chunk_number} chunks (timeout-managed)"
        )

        return first_chunk if first_chunk is not None else pd.DataFrame(), metadata

    def _execute_streaming_internal(
        self,
        source: StreamingDataSource,
        query_id: str,
        chunk_size: int,
        ...
        metadata = {
            ...
            "timeout_info": {"timeout_configured": False},
            "estimated_total_rows": source.estimate_total_rows(),
            "streaming": True,
            "buffer_complete": result_buffer.is_complete,
        }
    ...
    def _generate_token_metadata(
        self, sample_df: pd.DataFrame, total_rows: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            estimation = token_manager.estimate_tokens_for_query_result(
                total_rows, sample_df
            )
            ...
            return {
                ...
                "chunking_recommendation": {
                    "should_chunk": response_metadata.chunking_recommendation.should_chunk
                    if response_metadata.chunking_recommendation
                    else False,
                    "recommended_chunk_size": response_metadata.chunking_recommendation.recommended_chunk_size
                    if response_metadata.chunking_recommendation
                    else None,
                    "strategy": response_metadata.chunking_recommendation.strategy
                    if response_metadata.chunking_recommendation
                    else None,
                    "rationale": response_metadata.chunking_recommendation.chunk_size_rationale
                    if response_metadata.chunking_recommendation
                    else None,
                },
                "model_compatibility": {
    ...
    def get_chunk_iterator(
        self, query_id: str, start_row: int = 0, chunk_size: Optional[int] = None
    ...
    @staticmethod
    def _detect_db_type(source: "StreamingDataSource") -> str:
    ...
    def _adapt_chunk_size(
        ...
        recent_metrics = (
            self._chunk_metrics[-3:]
            if len(self._chunk_metrics) >= 3
            else self._chunk_metrics
        )
        ...
        # If memory per row is high, reduce chunk size
            ...
            logger.debug(
                f"Reducing chunk size due to high memory usage: {current_chunk_size} -> {new_chunk_size}"
            )
        ...
        # If processing is fast and memory is available, can increase chunk size
            ...
            logger.debug(
                f"Increasing chunk size due to good performance: {current_chunk_size} -> {new_chunk_size}"
            )
