...
class SecurityManager:
    ...
    def analyze_query_complexity(self, query: str) -> QueryComplexity:
        ...
        complexity.conditions = len(
            re.findall(r"\b(?:where|and|or|having)\b", query_lower)
        )
    ...
    def check_rate_limits(
        ...
        with self._rate_limit_lock:
            ...
            if len(rate_state.queries_this_minute) >= self.config.queries_per_minute:
                ...
                return (
                    False,
                    f"Rate limit exceeded: {len(rate_state.queries_this_minute)}/{self.config.queries_per_minute} queries per minute",
                )
            ...
            if len(rate_state.queries_this_hour) >= self.config.queries_per_hour:
                ...
                return (
                    False,
                    f"Rate limit exceeded: {len(rate_state.queries_this_hour)}/{self.config.queries_per_hour} queries per hour",
                )
    ...
    def check_resource_limits(self, database_name: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if len(db_operations) >= self.config.max_concurrent_queries:
                return (
                    False,
                    f"Maximum concurrent queries exceeded: {len(db_operations)}/{self.config.max_concurrent_queries}",
                )
    ...
    def validate_query_security(
        ...
        try:
            ...
            if connection_id:
                ...
                rate_allowed, rate_error = self.check_rate_limits(
            ...
            if complexity.length > self.config.max_query_length:
                ...
                return (
                    False,
                    f"Query too long: {complexity.length}/{self.config.max_query_length} characters",
                    security_metadata,
                )
            ...
            if complexity.joins > self.config.max_joins:
                ...
                return (
                    False,
                    f"Too many joins: {complexity.joins}/{self.config.max_joins}",
                    security_metadata,
                )
            ...
            if complexity.subqueries > self.config.max_subqueries:
                ...
                return (
                    False,
                    f"Too many subqueries: {complexity.subqueries}/{self.config.max_subqueries}",
                    security_metadata,
                )
            ...
            if self.config.enable_pattern_detection:
                ...
                security_metadata["attack_patterns"] = [
                    p.value for p in detected_patterns
                ]
                    ...
                    for pattern in detected_patterns:
                        self._log_security_event(
                            SecurityEvent(
                                ...
                                metadata={
                                    "all_patterns": [p.value for p in detected_patterns]
                                },
                            )
                        )
                ...
                try:
                    ...
                    with self._events_lock:
                        while (
                            ...
                            and self._security_events[0].timestamp < cutoff_time
                        ):
