...
from .config_manager import DatabaseConfig, DatabaseType, get_config_manager
...
class QueryTimeoutManager:
    ...
    def get_timeout_config(self, database_name: str) -> TimeoutConfig:
        ...
        if not db_config:
            ...
            logger.warning(
                f"Database '{database_name}' not found in config, using defaults"
            )
    ...
    def timeout_context(
        ...
        if timeout_config.query_timeout > 0:
            ...
            logger.info(
                f"Started timeout monitoring for operation '{operation_id}' "
            ...
            with self._operation_lock:
                if operation_id in self._active_operations:
                    ...
                    if op_context.get("cancelled"):
                        logger.warning(
                            f"Operation '{operation_id}' was cancelled after {execution_time:.2f}s"
                        )
                    else:
                        logger.info(
    ...
    def _monitor_timeout(self, operation_id: str, timeout_seconds: int) -> None:
        ...
        with self._operation_lock:
            ...
            if cleanup_func and timeout_config.cleanup_on_timeout:
                try:
                    logger.info(
                        f"Performing cleanup for timed out operation '{operation_id}'"
                    )
                    cleanup_func()
    ...
    def cancel_operation(
        self, operation_id: str, reason: TimeoutReason = TimeoutReason.MANUAL_CANCEL
    ) -> bool:
        ...
        with self._operation_lock:
            ...
            logger.info(
                f"Manually cancelled operation '{operation_id}' (reason: {reason.value})"
            )
            ...
            if cleanup_func and timeout_config.cleanup_on_timeout:
                try:
                    ...
                    logger.error(
                        f"Cleanup failed for cancelled operation '{operation_id}': {e}"
                    )
                    operation_context["cleanup_performed"] = False
        ...
        with self._operation_lock:
            ...
            for op_id, context in self._active_operations.items():
                ...
                result[op_id] = {
                    ...
                    "time_remaining": max(
                        0, timeout_config.query_timeout - (current_time - start_time)
                    ),
    ...
    def create_timeout_error(
        self,
        operation_id: str,
        timeout_reason: TimeoutReason = TimeoutReason.USER_TIMEOUT,
    ) -> QueryTimeoutError:
...
def with_timeout(
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            op_id = (
                operation_id or f"{database_name}_{func.__name__}_{int(time.time())}"
            )

            with timeout_manager.timeout_context(
                op_id, timeout_config, cleanup_func
            ) as context:
                ...
                def check_cancellation():
                    if timeout_manager.is_cancelled(op_id):
                        raise timeout_manager.create_timeout_error(
                            op_id, TimeoutReason.USER_TIMEOUT
                        )
