...
def probe(handle: EngineHandle, backend_kind: str) -> HealthCheckResult:
    ...
    """
    statement = _PROBE_SQL_ORACLE if backend_kind == "oracle" else _PROBE_SQL
...
def _execute_probe(connection: object, backend_kind: str, statement: str) -> None:
    ...
    native connection takes the string directly; SQLAlchemy connections
    take a `text()` construct."""
    if backend_kind == "duckdb":
