...
class ResponseMetadataGenerator:
        ...
        for col in df.columns:
            if df[col].dtype == "object":
                ...
                consistency_scores.append(
                    1.0
                )  # Numeric columns are consistent by definition
    ...
    def _generate_schema_information(
        self, df: pd.DataFrame, db_name: Optional[str] = None
    ) -> SchemaInformation:
    ...
    def _calculate_memory_footprint(
        self, df: pd.DataFrame, token_estimation: TokenEstimation
    ) -> float:
    ...
    def _generate_chunk_availability(
        self, df: pd.DataFrame, token_estimation: TokenEstimation
    ) -> ChunkAvailability:
    ...
    def _determine_recommended_action(
        ...
        if token_estimation.total_tokens > 100000:
            return (
                "chunk",
                f"Dataset is very large ({token_estimation.total_tokens:,} tokens). Recommend chunking for better handling.",
            )

        elif token_estimation.total_tokens > 20000:
            return (
                "sample",
                f"Dataset is large ({token_estimation.total_tokens:,} tokens). Consider sampling or chunking.",
            )

        elif data_quality.overall_quality == DataQualityLevel.POOR:
            return (
                "review",
                f"Data quality is {data_quality.overall_quality.value} (score: {data_quality.quality_score:.2f}). Review before processing.",
            )

        elif complexity_level == QueryComplexity.INTENSIVE:
            return (
                "stream",
                f"Query complexity is {complexity_level.value}. Consider streaming processing.",
            )

        else:
            return (
                "proceed",
                f"Dataset is manageable ({token_estimation.total_tokens:,} tokens, {row_count:,} rows). Safe to proceed.",
            )

    def _generate_llm_friendly_summary(
        self,
        df: pd.DataFrame,
        token_estimation: TokenEstimation,
        data_quality: DataQualityMetrics,
        ...
        size_desc = (
            "small" if len(df) < 1000 else "medium" if len(df) < 10000 else "large"
        )
