...
class TypeInferenceEngine:
    ...
    def __init__(self):
        ...
        self.confidence_thresholds = {"high": 0.9, "medium": 0.7, "low": 0.5}
    ...
    def infer_type(self, data: pd.Series) -> TypeInferenceResult:
        ...
        suggestions = self._generate_conversion_suggestions(
            inference_results, current_dtype
        )
        ...
        return TypeInferenceResult(
            ...
            sample_analysis={
                ...
                "inference_scores": {
                    k.value: v["confidence"] for k, v in inference_results.items()
                },
            },
    ...
    def _test_integer(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            integer_mask = (
                numeric_data == numeric_data.astype("Int64", errors="ignore")
            ).fillna(False)
    ...
    def _test_datetime(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            datetime_data = pd.to_datetime(
                data.astype(str), errors="coerce", infer_datetime_format=True
            )
            ...
            return {
                ...
                "details": {
                    ...
                    "date_range": (
                        (datetime_data.min(), datetime_data.max())
                        if valid_count > 0
                        else None
                    ),
                },
        ...
        try:
            ...
            return {
                ...
                "details": {
                    ...
                    "date_range": (
                        date_data.min() if valid_count > 0 else None,
    ...
    def _test_currency(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            str_data = data.astype(str).str.strip()
            currency_matches = str_data.str.match(
                self.patterns[DataType.CURRENCY]
            ).sum()
    ...
    def _generate_conversion_suggestions(
        self, inference_results: Dict, current_dtype: str
    ) -> List[Dict[str, Any]]:
        ...
        sorted_results = sorted(
            inference_results.items(), key=lambda x: x[1]["confidence"], reverse=True
        )
    ...
    def _get_conversion_risks(
        self, data_type: DataType, current_dtype: str
    ) -> List[str]:
    ...
    def _identify_potential_issues(
        self, data: pd.Series, inferred_type: DataType
    ) -> List[str]:
        ...
        if inferred_type in [DataType.INTEGER, DataType.FLOAT]:
            try:
                ...
                if conversion_failures > 0:
                    issues.append(
                        f"{conversion_failures} values cannot be converted to numeric"
                    )
            except:
                pass
    ...
    def _calculate_memory_impact(
        ...
        return {
            ...
            "change_percentage": (
                ((estimated_memory - current_memory) / current_memory) * 100
                if current_memory > 0
                else 0
            ),
        }
...
class DataTypeManager(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for column in X.columns:
            if column not in self.custom_type_mappings and self.auto_inference:
                ...
                logger.info(
                    f"Inferred type for column '{column}'",
    ...
    def transform(self, X):
        ...
        for column in X.columns:
            try:
                ...
                if column in self.custom_type_mappings:
                    ...
                    if (
                        inference_result.confidence_score > 0.7
        ...
        execution_time = time.time() - start_time
        logger.info(
            "DataTypeManager transform completed",
            execution_time=execution_time,
            conversions_applied=len(
                [op for op in self.conversion_history if op.success]
            ),
        )
    ...
    def _convert_column(
        self, column: pd.Series, column_name: str, target_type: DataType
    ) -> Tuple[pd.Series, ConversionOperation]:
        """
        ...
        try:
            ...
            if self.validation_enabled:
                conversion_errors = self._validate_conversion(
                    column, converted_column, target_type
                )
    ...
    def _convert_to_date(self, column: pd.Series) -> pd.Series:
        ...
        datetime_column = pd.to_datetime(
            column, errors="coerce", infer_datetime_format=True
        )
    ...
    def _validate_conversion(
        self, original: pd.Series, converted: pd.Series, target_type: DataType
    ) -> int:
        """
    ...
    def get_memory_impact_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_memory_change_per_column": (
                total_memory_saved / successful_conversions
    ...
    def get_conversion_summary(self) -> str:
        ...
        if type_counts:
            conversion_details = ", ".join(
                [f"{count} to {type_name}" for type_name, count in type_counts.items()]
            )
