...
class DataInputPipeline(AnalysisPipelineBase):
    ...
    def _execute_standard_analysis(
        ...
        metadata = {
            ...
            "memory_usage_mb": result_data.memory_usage(deep=True).sum()
            / (1024 * 1024),
    ...
    def _validate_data_source(self, data: Any, context: Dict[str, Any]) -> bool:
        ...
        try:
            if self._data_source_type == DataSourceType.DATAFRAME:
            ...
            elif self._data_source_type in [
                DataSourceType.CSV_FILE,
                DataSourceType.EXCEL_FILE,
                DataSourceType.JSON_FILE,
                DataSourceType.PARQUET_FILE,
            ]:
                ...
                if not any(
                    ...
                    for cmd in ["SELECT", "WITH", "SHOW", "EXPLAIN"]
                ):
                ...
                missing = [
                    field for field in required_fields if field not in self.data_source
                ]
                if missing:
                    raise ValueError(
                        f"Database connection missing required fields: {missing}"
                    )
                return True
    ...
    def _create_streaming_source(
        ...
        try:
            if self._data_source_type == DataSourceType.DATAFRAME:
                ...
                self._streaming_source = create_streaming_source(
                    engine=engine, query=self.data_source
                )
                ...
                metadata.update(
                    {
                        ...
                        "estimated_complexity": self._estimate_query_complexity(
                            self.data_source
                        ),
                    }
                )
    ...
    def _build_input_metadata(
        self, sample_data: pd.DataFrame, streaming_metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        base_metadata = {
            ...
            "data_characteristics": {
                ...
                "datetime_columns": sample_data.select_dtypes(
                    include=["datetime64"]
                ).columns.tolist(),
            },
    ...
    def _is_file_path(self, source: str) -> bool:
        ...
        return Path(source).exists() or any(
    ...
    def _get_sql_engine(self) -> Engine:
        ...
        raise NotImplementedError(
            "SQL engine integration with connection manager needed"
        )
