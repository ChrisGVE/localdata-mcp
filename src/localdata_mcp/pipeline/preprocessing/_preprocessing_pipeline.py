"""
Data Preprocessing Pipeline - Progressive disclosure preprocessing.

Implements the DataPreprocessingPipeline class with configurable complexity
levels (minimal, auto, comprehensive, custom) and streaming support.
"""
...
class DataPreprocessingPipeline(AnalysisPipelineBase):
    ...
    def __init__(
        ...
        logger.info(
            "DataPreprocessingPipeline initialized",
    ...
    def _configure_analysis_pipeline(self) -> List[Callable]:
        ...
        if self.preprocessing_intent == PreprocessingIntent.MINIMAL:
            pipeline_steps.extend(
                [self._handle_missing_values, self._infer_and_convert_types]
            )
        ...
        logger.info(
            f"Configured preprocessing pipeline with {len(pipeline_steps)} steps"
        )
    ...
    def _execute_analysis_step(
        ...
        try:
            ...
            return (
                data,
                metadata,
            )  # Return original data, don't fail the entire pipeline
    ...
    def _handle_missing_values(
        ...
        strategy = kwargs.get(
            "strategy", TransformationStrategy.missing_values_auto(data)
        )
    ...
    def _detect_and_handle_outliers(
        ...
        strategy = kwargs.get(
            "strategy", TransformationStrategy.outlier_detection_auto(data)
        )
            ...
            if strategy == "iqr":
                ...
                if action == "cap":
                    ...
                    result_data[col] = np.clip(
                        data[col], mean_val - 3 * std_val, mean_val + 3 * std_val
                    )
                elif action == "remove":
                    result_data = result_data[~outlier_mask]
                elif action == "flag":
        ...
        metadata = {
            ...
            "total_outliers_processed": sum(
                log["outliers_detected"] for log in outlier_log.values()
            ),
        }
    ...
    def _encode_categorical_variables(
        ...
        strategy = kwargs.get(
            "strategy", TransformationStrategy.encoding_strategy_auto(data)
        )
        ...
        for col in categorical_cols:
            ...
            if strategy == "onehot" or (strategy == "auto" and cardinality <= 10):
                ...
                result_data = pd.concat(
                    [result_data.drop(col, axis=1), dummies], axis=1
                )
                encoding_log[col] = {
                    "method": "onehot",
    ...
    def _dimensionality_assessment(
        self, data: pd.DataFrame, **kwargs
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        samples_to_features_ratio = (
            shape[0] / len(numeric_cols) if len(numeric_cols) > 0 else float("inf")
        )
        ...
        if samples_to_features_ratio < 10:
            recommendations.append(
                "Low samples-to-features ratio - consider feature selection"
            )
    ...
    def _build_preprocessing_metadata(
        ...
        original_shape = self._execution_context.get("data_profile", {}).get(
            "shape", (0, 0)
        )
    ...
    def _calculate_data_quality_score(self, data: pd.DataFrame) -> float:
        ...
        type_validity = (
            85  # Base score, could be enhanced with more sophisticated checks
        )
    ...
    def _extract_preprocessing_artifacts(self) -> Dict[str, Any]:
        ...
        artifacts["transformation_statistics"] = {
            ...
            "successful_steps": sum(
                1 for log in self._preprocessing_log if log["status"] == "success"
            ),
            "failed_steps": sum(
                1 for log in self._preprocessing_log if log["status"] == "error"
            ),
        }
