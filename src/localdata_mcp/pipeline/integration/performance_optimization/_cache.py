"""
Conversion Cache System for the Performance Optimization sub-package.

Provides intelligent caching with LRU eviction, memory-aware management,
and performance monitoring for repeated conversion operations.
"""
...
class ConversionCache:
    ...
    def __init__(
        ...
        self._cleanup_executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="cache_cleanup"
        )

        logger.info(
            f"ConversionCache initialized",
            max_size=max_size,
            max_memory_mb=max_memory_mb,
            ttl_seconds=ttl_seconds,
            policy=eviction_policy.value,
        )
            ...
            if HAS_PANDAS and isinstance(data, pd.DataFrame):
                ...
                if len(data) > 0:
                    ...
                    sample_data = pd.concat(
                        [data.head(sample_size), data.tail(sample_size)]
                    ).fillna("NULL")
                    fingerprint_data["sample_hash"] = str(
                        hash(str(sample_data.values.tobytes()))
                    )
    ...
    def get(self, request: ConversionRequest) -> Optional[CachedConversion]:
        ...
        with self._cache_lock:
            ...
            if cache_key in self._cache:
                ...
                logger.debug(
                    f"Cache hit for key {cache_key[:8]}... (lookup: {lookup_time:.2f}ms)"
                )
                return cached
    ...
    def put(self, request: ConversionRequest, result: ConversionResult) -> None:
        ...
        if self.enable_compression and estimated_size_mb > 10.0:
            try:
                ...
                logger.debug(
                    f"Compressed cache entry {cache_key[:8]}... from {estimated_size_mb:.1f}MB"
                )
            except Exception as e:
        ...
        with self._cache_lock:
            ...
            logger.debug(
                f"Cached conversion {cache_key[:8]}... ({estimated_size_mb:.1f}MB)"
            )
    ...
    def invalidate(self, pattern: str = None) -> int:
        ...
        with self._cache_lock:
            ...
            for cache_key in self._cache:
                ...
                if pattern in cache_key or pattern in str(
    ...
    def _estimate_result_size(self, result: ConversionResult) -> float:
        ...
        try:
            if HAS_PANDAS and isinstance(result.converted_data, pd.DataFrame):
                return result.converted_data.memory_usage(deep=True).sum() / (
                    1024 * 1024
                )
    ...
    def _evict_to_fit(self, required_mb: float) -> None:
        ...
        while self._current_memory_mb > target_memory and len(self._cache) > 0:
            self._evict_oldest()
    ...
    def _evict_oldest(self) -> None:
        ...
        if self.eviction_policy == CacheEvictionPolicy.LRU:
            ...
            oldest_key = min(
                self._cache.keys(), key=lambda k: self._cache[k].creation_time
            )

        else:  # MEMORY_PRESSURE - remove largest item
            oldest_key = max(self._cache.keys(), key=lambda k: self._cache[k].size_mb)
    ...
    def _record_lookup_time(self, lookup_time_ms: float) -> None:
        ...
        if self._lookup_times:
            self.stats.average_lookup_time_ms = sum(self._lookup_times) / len(
                self._lookup_times
            )
