...
class AnalysisPipelineBase(BaseEstimator, TransformerMixin, ABC):
    ...
    def fit(self, X: pd.DataFrame, y=None) -> "AnalysisPipelineBase":
        ...
        try:
            ...
            if self.composition_aware:
                self._composition_metadata = self._build_composition_context(
                    X, data_profile
                )
    ...
    def transform(self, X: pd.DataFrame) -> PipelineResult:
        ...
        try:
            ...
            error_result = self._handle_pipeline_error(
                e,
                "transform",
                context={"data_shape": X.shape, "execution_time": execution_time},
    ...
    def _should_enable_streaming(
        ...
        return (
            data_size_mb > self.streaming_config.threshold_mb
            or len(data) > 100000  # Large row count
            or self.streaming_config.enabled
        )  # Explicitly enabled
    ...
    def _configure_streaming(self, data: pd.DataFrame, profile: Dict[str, Any]):
        ...
        if self.streaming_config.chunk_size_adaptive:
            estimated_row_size = (
                profile["memory_usage_mb"] / len(data) * 1024 * 1024
            )  # bytes per row
            target_chunk_memory_mb = min(
                self.streaming_config.memory_limit_mb // 4, 250
            )
            adaptive_chunk_size = int(
                target_chunk_memory_mb * 1024 * 1024 / estimated_row_size
            )
            self.streaming_config.initial_chunk_size = max(
                min(adaptive_chunk_size, 10000), 100
            )
