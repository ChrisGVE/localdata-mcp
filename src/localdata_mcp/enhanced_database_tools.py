...
class EnhancedDatabaseTools:
    ...
    def initialize_all_configured_databases(self) -> Dict[str, bool]:
        ...
        for name, config in db_configs.items():
            if config.enabled:
                logger.info(
                    f"Initializing database '{name}' (type: {config.type.value})"
                )
    ...
    def get_database_status(self, name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if name:
            ...
            status = {"total_configured": len(all_configs), "databases": {}}
    ...
    def execute_enhanced_query(
        ...
        try:
            ...
            connection_id = (
                connection_id or f"enhanced_db_tools_{int(time.time() * 1000)}"
            )
            is_secure, security_error, security_metadata = (
                self.security_manager.validate_query_security(
            ...
            if not db_config:
                return {
                    "error": f"Database configuration not found for '{database_name}'"
                }
            ...
            if enable_analysis:
                try:
                    query_analysis = analyze_query(
                        validated_query, engine, database_name
                    )
            ...
            with self.security_manager.secure_query_execution(
                validated_query, database_name, connection_id
            ) as security_context:
                with self.connection_manager.managed_query_execution(
                    database_name, query_id
                ) as context:
                    ...
                    initial_chunk_size = chunk_size or (
                        query_analysis.recommended_chunk_size
                        if query_analysis and query_analysis.should_chunk
                        else 100
                    )

                    # Execute with timeout management
                    timeout_config = self.timeout_manager.get_timeout_config(
                        database_name
                    )
                    ...
                    try:
                        ...
                        if first_chunk.empty:
                            ...
                            total_rows = streaming_metadata.get(
                            ...
                            result = {
                                ...
                                "data": json.loads(
                                    first_chunk.to_json(orient="records")
                                ),
                        ...
                        result["security"] = {
                            ...
                            "attack_patterns": security_metadata.get(
                                "attack_patterns", []
                            ),
                        }
...
def connect_enhanced_database(name: str) -> str:
    ...
    try:
            ...
            return json.dumps(
                {
                    ...
                    "available_databases": list(
                        enhanced_tools.config_manager.get_database_configs().keys()
                    ),
                }
            )
        ...
        if success:
            ...
            return json.dumps(
                {"success": False, "error": f"Failed to initialize database '{name}'"}
            )
...
def execute_enhanced_query(
    ...
    result = enhanced_tools.execute_enhanced_query(
        name, query, chunk_size, connection_id=connection_id
    )
...
def list_databases_by_tags(tags: str) -> str:
    ...
    return json.dumps(
        {"filters": tag_list, "matching_databases": databases, "count": len(databases)},
        indent=2,
    )
...
def get_security_events(
    ...
    """
    try:
        from .security import SecurityEventType, SecurityThreatLevel
        ...
        if event_type:
            try:
                ...
                return json.dumps(
                    {
                        "error": f"Invalid event_type: {event_type}. Valid types: {', '.join([e.value for e in SecurityEventType])}"
                    }
                )
        ...
        if threat_level:
            try:
                ...
                return json.dumps(
                    {
                        "error": f"Invalid threat_level: {threat_level}. Valid levels: {', '.join([t.value for t in SecurityThreatLevel])}"
                    }
                )

        events = enhanced_tools.security_manager.get_security_events(
            limit=limit, event_types=event_types, threat_levels=threat_levels
        )
        ...
        return json.dumps(
            {"total_events": len(events_data), "events": events_data},
            indent=2,
            default=str,
        )
...
@enhanced_mcp.tool
def validate_query_security_standalone(
    ...
    try:
        is_valid, error_msg, metadata = (
            enhanced_tools.security_manager.validate_query_security(
