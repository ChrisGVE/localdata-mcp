...
from .streaming import StreamingQueryExecutor, create_streaming_source, MemoryStatus
...
class PerformanceBenchmarkSuite:
    ...
    def _run_regression_tests(self):
        ...
        if not baseline_path.exists():
            logger.info(
                "No baseline results found, current results will become baseline"
            )
            return
        ...
        try:
            ...
            for current_result in self.results:
                baseline_result = self._find_matching_baseline(
                    current_result, baseline_results
                )
                ...
                if (
                    ...
                    if (
                        comparison.execution_time_improvement_percent < -10
                    ):  # 10% slower
                        logger.warning(
    ...
    def _find_matching_baseline(
        self, current_result: BenchmarkResult, baseline_results: List[BenchmarkResult]
    ...
    def _save_detailed_results(self):
        ...
        results_file = (
            self.config.results_dir / f"benchmark_results_{int(time.time())}.json"
        )
        with open(results_file, "w") as f:
            json.dump(
                [result.to_dict() for result in self.results], f, indent=2, default=str
            )
        ...
        if self.comparisons:
            comparisons_file = (
                ...
                / f"benchmark_comparisons_{int(time.time())}.json"
            )
            with open(comparisons_file, "w") as f:
                json.dump(
                    [asdict(comp) for comp in self.comparisons],
                    f,
                    indent=2,
                    default=str,
                )
    ...
    def _generate_reports(self):
        ...
        report_file = (
            self.config.results_dir / f"performance_report_{int(time.time())}.md"
        )
        ...
        with open(report_file, "w") as f:
            ...
            f.write(
                f"- Successful tests: {sum(1 for r in self.results if r.success)}\n"
            )
            ...
            for category in categories:
                ...
                category_results = [
                    r for r in self.results if r.test_category == category and r.success
                ]
                if category_results:
                    avg_rate = sum(
            ...
            if self.comparisons:
                ...
                for comp in self.comparisons:
                    ...
                    f.write(
                        f"- Execution time improvement: {comp.execution_time_improvement_percent:.1f}%\n"
                    )
                    f.write(
                        f"- Memory improvement: {comp.memory_improvement_percent:.1f}%\n"
                    )
                    f.write(
                        f"- Processing rate improvement: {comp.processing_rate_improvement_percent:.1f}%\n"
                    )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        streaming_results = [
            r for r in self.results if r.processing_mode == "streaming" and r.success
        ]
        batch_results = [
            r for r in self.results if r.processing_mode == "batch" and r.success
        ]
        ...
        if streaming_results and batch_results:
            ...
            if avg_streaming_rate > avg_batch_rate * 1.1:
                recommendations.append(
                    "Streaming processing shows significant performance benefits - recommend for large datasets"
                )
            elif avg_batch_rate > avg_streaming_rate * 1.1:
                recommendations.append(
                    "Batch processing performs better for current workloads - streaming may have overhead"
                )
            else:
                recommendations.append(
                    "Streaming and batch processing show similar performance - choose based on memory constraints"
                )
        ...
        memory_results = [
            ...
            if r.test_category == "memory_efficiency" and r.success
        ]
        if memory_results:
        ...
        token_results = [
            r
            for r in self.results
            if r.test_category == "token_performance" and r.success
        ]
        if token_results:
            avg_token_rate = sum(
        ...
        return {
            ...
            "performance_improvements": len(
                [
                    ...
                    if c.execution_time_improvement_percent > 5
                ]
            ),
