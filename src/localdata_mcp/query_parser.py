...
class QueryParser:
    ...
    def _compile_security_patterns(self) -> None:
        ...
        self.blocked_pattern = re.compile(rf"\b(?:{blocked_ops})\b", re.IGNORECASE)

        # Pattern to detect allowed operations at the start of statements
        allowed_ops = "|".join(self.ALLOWED_OPERATIONS.keys())
        self.allowed_pattern = re.compile(rf"^\s*(?:{allowed_ops})\b", re.IGNORECASE)
    ...
    def validate_query(self, query: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if self._check_multiple_statements(normalized_query):
                return (
                    False,
                    "Multiple SQL statements in a single query are not allowed for security reasons",
                )
            ...
            if not self._is_select_operation(normalized_query):
                return (
                    False,
                    "Only SELECT queries and Common Table Expressions (WITH) are allowed",
                )
            ...
            if blocked_ops:
                ...
                return (
                    False,
                    f"Query contains blocked operations: {blocked_list}. Only SELECT queries are allowed.",
                )
    ...
    def parse_and_validate(self, query: str) -> str:
        ...
        logger.debug(
            f"SQL query validation passed: {query[:100]}{'...' if len(query) > 100 else ''}"
        )
        return query
...
# Patterns that look like SELECT but actually write data
_READONLY_PATTERNS = [
    r"\bSELECT\s+.*\s+INTO\b",
    r"\bCREATE\s+TABLE\s+.*\s+AS\s+SELECT\b",
    r"\bCOPY\s+.*\s+TO\b",
    r"\bINSERT\s+ALL\b",
    r"\bMERGE\s+INTO\b",
    r"\bOUTPUT\s+INTO\b",
...
def check_readonly(query: str) -> Optional[str]:
    """Return the blocked pattern if query violates readonly mode, else None."""
    upper = query.upper()
