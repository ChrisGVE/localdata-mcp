"""LocalData MCP - DatabaseManager class and QueryBuffer."""
...
class DatabaseManager(SamplingToolsMixin, OptimizationToolsMixin, GeospatialToolsMixin):
    def __init__(self):
        ...
        self._register_tools(_parent.mcp)
    ...
    def _register_tools(self, mcp_server):
        ...
        mcp_server.add_tool(self.generate_sample)
    ...
    def _sanitize_path(self, file_path: str):
        ...
        try:
            sec_cfg = _parent.get_config_manager().get_security_config()
        except Exception:
            sec_cfg = None
    ...
    def _get_engine(
        ...
        if db_type == "sqlite":
            ...
            if not _parent.DUCKDB_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.REDIS_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.ELASTICSEARCH_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.PYMONGO_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.INFLUXDB_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.NEO4J_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.COUCHDB_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.ORACLEDB_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.MSSQL_AVAILABLE:
                raise ValueError(
            ...
            if not _parent.SPARQLWRAPPER_AVAILABLE:
                raise ValueError(
    ...
    def _create_engine_from_file(
        ...
        try:
            ...
            if file_type == "yaml":
                ...
                if not _parent.PYARROW_AVAILABLE:
                    raise ValueError(
    ...
    def _load_excel_file(
        ...
        if not _parent.DEFUSEDXML_AVAILABLE:
            logger.warning(
        ...
        try:
            ...
            if file_ext in [".xlsx", ".xlsm"]:
                # Modern Excel format
                if not _parent.OPENPYXL_AVAILABLE:
                    raise ValueError(
    ...
    def _load_ods_file(
        ...
        if not _parent.ODFPY_AVAILABLE:
    ...
    def _load_numbers_file(
        ...
        if not _parent.NUMBERS_PARSER_AVAILABLE:
            logger.warning(
    ...
    def disconnect_database(self, name: str):
        ...
        try:
            ...
            staging_cleaned = _parent.get_staging_manager().cleanup_by_parent(name)
    ...
    def list_databases(self, include_staging: bool = False) -> str:
        ...
        if include_staging:
            staging_entries = _parent.get_staging_manager().list_staging()
