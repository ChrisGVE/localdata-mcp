...
class TimeSeriesTransformer(BaseEstimator, TransformerMixin, ABC):
    ...
    def _validate_time_series(
        ...
        if not isinstance(X.index, pd.DatetimeIndex):
            if (
                "datetime" in X.columns
                or "date" in X.columns
                or "timestamp" in X.columns
            ):
    ...
    def _detect_seasonality(
        ...
        try:
            ...
            if (
                best_period is not None and best_strength > 0.3
    ...
    def fit(
        ...
        try:
            ...
            for i, transformer in enumerate(self.transformers):
                logger.debug(
                    f"Fitting transformer {i + 1}/{len(self.transformers)}: {type(transformer).__name__}"
                )
...
class TimeSeriesResamplingTransformer(TimeSeriesTransformer):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None):
        ...
        logger.debug(
            f"Fitted resampling transformer: {self.original_frequency_} -> {self.target_frequency}"
        )
        return self
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if isinstance(self.aggregation_method, dict):
                X_resampled = X.resample(self.target_frequency).agg(
                    ...
                    if col in self.seasonal_patterns_:
                        ...
                        for idx in missing_mask[missing_mask].index:
                            ...
                            if position_in_period in seasonal_means:
                                X_imputed.loc[idx, col] = seasonal_means[
                                    position_in_period
                                ]
    ...
    def transform(self, X: pd.DataFrame) -> TimeSeriesAnalysisResult:
        ...
        try:
            ...
            if missing_pct > self.max_missing_percentage * 100:
                quality_issues.append(
                    f"High missing values: {missing_pct:.1f}% > {self.max_missing_percentage * 100:.1f}% threshold"
                )
                recommendations_list.append("Apply missing value imputation")

            # Check frequency regularity
            if (
                self.require_regular_frequency
                and quality_assessment["frequency"] is None
            ):
            ...
            if len(quality_issues) == 0:
                interpretation = (
                    f"Time series data quality: GOOD (score: {overall_quality:.2f})"
                )
            ...
            if "kpss" in self.tests:
                try:
                    ...
                    test_results["kpss"] = {
                        ...
                        "is_stationary": kpss_result[1]
                        > self.alpha,  # Note: KPSS null hypothesis is stationarity
                        "interpretation": "Stationary"
                        if kpss_result[1] > self.alpha
                        else "Non-stationary",
                    }
            ...
            if self.test_type == "adf":
                try:
                    ...
                    interpretation = (
                        ...
                        if has_unit_root
                        else "Series does not have unit root (stationary)"
                    )
                ...
                try:
                    result = kpss(
                        series, regression=self.regression_type, nlags=self.autolag
                    )
                    test_results["kpss"] = {
                        "test_statistic": result[0],
                        "p_value": result[1],
                ...
                try:
                    result = pp_test(
                        series, lags=self.autolag, regression=self.regression_type
                    )
                    test_results["pp"] = {
                        "test_statistic": result[0],
                        "p_value": result[1],
                ...
                try:
                    ...
                    if len(diff_series) >= 10:
                        ...
                        if diff_result[1] <= 0.05:
                            ...
                            recommendations.append(
                                "May require second differencing or seasonal differencing (I(2) or seasonal process)"
                            )
                except Exception:
                    pass
            else:
...
def validate_time_series_continuity(
    ...
    try:
        ...
        quality_factors = [
            ...
            max(
                0.0, 1.0 - result["missing_value_percentage"] / 50.0
...
class ImpulseResponseAnalyzer(MultivariateTimeSeriesTransformer):
    ...
    def _analysis_logic(self, X: pd.DataFrame) -> TimeSeriesAnalysisResult:
        ...
        try:
            ...
            logger.info(
                f"Fitted VAR({optimal_lags}) model for impulse response analysis"
            )
            ...
            irf_df = pd.DataFrame(
                {key: values for key, values in irf_dict.items()}, index=periods_index
            )
            ...
            if cumulative_responses is not None:
                ...
                for shock_idx, shock_var in enumerate(series_names):
                    for response_idx, response_var in enumerate(series_names):
                        key = f"{shock_var} → {response_var}"
                        cumulative_dict[key] = cumulative_responses[
                            :, response_idx, shock_idx
                        ]

                cumulative_irf_df = pd.DataFrame(cumulative_dict, index=periods_index)
                    ...
                    for shock_idx, shock_var in enumerate(series_names):
                        for response_idx, response_var in enumerate(series_names):
                            ...
                            if key not in fevd_dict:
                                fevd_dict[key] = []
                            fevd_dict[key].append(
                                fevd_decomposition[response_idx, period, shock_idx]
                            )
                ...
                significant_responses.append(
                    {
                        ...
                        "total_cumulative_effect": np.sum(responses)
            ...
            significant_responses.sort(
                key=lambda x: abs(x["max_response"]), reverse=True
            )
