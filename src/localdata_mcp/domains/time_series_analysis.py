...
class StructuralBreakTester(TimeSeriesTransformer):
    ...
    def _calculate_test_diagnostics(self, series: pd.Series, test_results: Dict) -> Dict:
        ...
        if self.test_type == 'chow':
            ...
            diagnostics['rss_reduction'] = (
                test_results.get('rss_full', 0) - test_results.get('rss_unrestricted', 0)
            )
        elif self.test_type == 'cusum':
            diagnostics['cusum_boundary_violations'] = len(test_results.get('break_points', []))
            
        return diagnostics
    
    def _generate_break_test_interpretation(self, test_results: Dict, series: pd.Series, 
        ...
        if len(break_points) == 0:
            return (f"{self.test_type.title()} test found no evidence of structural breaks "
                   f"(test statistic: {statistic:.3f}, p-value: {p_value:.3f}). "
                   f"The time series appears structurally stable over the analyzed period.")
        
        interpretation = (f"{self.test_type.title()} test detected {len(break_points)} "
    ...
    def _generate_break_test_recommendations(self, test_results: Dict, series: pd.Series,
        ...
        if len(break_points) == 0:
            ...
            if len(break_points) > 1:
                recommendations.append("Multiple breaks suggest highly unstable system - use caution in forecasting")
        ...
        if self.test_type == 'chow':
            ...
            recommendations.append("CUSUM path analysis can help identify gradual vs. abrupt changes")
        
        # Power and sample size recommendations
        if len(series) < 50:
            recommendations.append("Small sample size may reduce test power - interpret results cautiously")
...
class SeasonalAnomalyDetector(TimeSeriesTransformer):
    ...
    def __init__(self, seasonal_period=None, method='adaptive_threshold', 
                 threshold_factor=2.5, adaptation_rate=0.1, min_history=50, **kwargs):
        super().__init__(**kwargs)
        self.seasonal_period = seasonal_period
        self.method = method
    ...
    def transform(self, X: pd.DataFrame) -> TimeSeriesAnalysisResult:
        ...
        try:
            ...
            if len(series) < self.min_history:
                raise ValueError(f"Insufficient data points for seasonal anomaly detection (need at least {self.min_history})")
            ...
            if period is None or period < 2:
                ...
                if self.method == 'adaptive_threshold':
                    anomalies, thresholds = self._detect_adaptive_threshold_anomalies(series, seasonal_info, period)
                elif self.method == 'seasonal_iqr':
                    anomalies, thresholds = self._detect_seasonal_iqr_anomalies(series, seasonal_info, period)
                elif self.method == 'seasonal_zscore':
                    anomalies, thresholds = self._detect_seasonal_zscore_anomalies(series, seasonal_info, period)
                else:
                    raise ValueError(f"Unknown seasonal anomaly detection method: {self.method}")
            ...
            anomaly_stats = self._calculate_seasonal_anomaly_statistics(series, anomalies, period)
            
            # Analyze seasonal patterns
            seasonal_analysis = self._analyze_seasonal_patterns(series, anomalies, seasonal_info, period)
            ...
            model_parameters = {
                ...
                'adaptive_thresholds': thresholds.tolist() if thresholds is not None else [],
    ...
    def _detect_non_seasonal_anomalies(self, series: pd.Series) -> Tuple[List[int], np.ndarray]:
        """Fallback to non-seasonal anomaly detection."""
    ...
    def _detect_adaptive_threshold_anomalies(self, series: pd.Series, 
                                           seasonal_info: Dict, period: int) -> Tuple[List[int], np.ndarray]:
    ...
    def _detect_seasonal_iqr_anomalies(self, series: pd.Series, 
                                     seasonal_info: Dict, period: int) -> Tuple[List[int], np.ndarray]:
    ...
    def _detect_seasonal_zscore_anomalies(self, series: pd.Series, 
                                        seasonal_info: Dict, period: int) -> Tuple[List[int], np.ndarray]:
    ...
    def _calculate_seasonal_anomaly_statistics(self, series: pd.Series, 
                                             anomalies: List[int], period: Optional[int]) -> Dict:
        ...
        if period and anomalies:
            ...
            stats['most_anomalous_season'] = max(seasonal_counts, key=seasonal_counts.get)
            stats['least_anomalous_season'] = min(seasonal_counts, key=seasonal_counts.get)
        
        return stats
    
    def _analyze_seasonal_patterns(self, series: pd.Series, anomalies: List[int],
                                 seasonal_info: Optional[Dict], period: Optional[int]) -> Dict:
        ...
        if len(anomalies) > 1:
            ...
            analysis['temporal_regularity'] = np.std(gaps) / np.mean(gaps) if np.mean(gaps) > 0 else 0
        
        return analysis
    
    def _generate_seasonal_interpretation(self, anomalies: List[int], series: pd.Series,
                                        seasonal_info: Optional[Dict], period: Optional[int],
                                        stats: Dict) -> str:
        ...
        if period:
            interpretation = (f"Detected {n_anomalies} seasonal anomal{'y' if n_anomalies == 1 else 'ies'} "
            ...
            interpretation = (f"Detected {n_anomalies} anomal{'y' if n_anomalies == 1 else 'ies'} "
                             f"({anomaly_rate:.1%} of observations) using non-seasonal method. ")
        
        # Describe frequency
        if anomaly_rate > 0.1:
            interpretation += "High frequency of anomalies suggests systematic seasonal issues. "
        elif anomaly_rate > 0.05:
            interpretation += "Moderate frequency indicates occasional seasonal disruptions. "
        else:
            interpretation += "Low frequency suggests rare exceptional seasonal events. "
        
        return interpretation
    
    def _generate_seasonal_recommendations(self, anomalies: List[int], series: pd.Series,
                                         seasonal_info: Optional[Dict], period: Optional[int],
        ...
        if period:
            recommendations.append(f"Seasonal patterns detected with period {period} - anomaly detection adjusted accordingly")
        ...
        if self.adaptation_rate > 0:
            recommendations.append("Adaptive thresholds are learning from recent patterns")
        else:
            recommendations.append("Consider enabling threshold adaptation for dynamic environments")
        
        # Method-specific recommendations
        if self.method == 'adaptive_threshold':
            recommendations.append("Adaptive threshold method provides good balance of sensitivity and stability")
        elif self.method == 'seasonal_iqr':
            recommendations.append("IQR method provides robust detection against outliers")
        elif self.method == 'seasonal_zscore':
            recommendations.append("Z-score method assumes normal distribution of seasonal residuals")
