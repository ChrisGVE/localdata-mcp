    ...
    def _prepare_time_series(self, X) -> pd.Series:
        ...
        if isinstance(X, pd.Series):
            if isinstance(X.index, pd.DatetimeIndex):
                ...
                return pd.Series(
                    X.values,
                    index=pd.date_range(start="2020-01-01", periods=len(X), freq="D"),
                )
            ...
            if self.date_column and self.value_column:
                if self.date_column in X.columns and self.value_column in X.columns:
                    ...
                    df_copy[self.date_column] = pd.to_datetime(
                        df_copy[self.date_column]
                    )
            ...
            return pd.Series(
                X, index=pd.date_range(start="2020-01-01", periods=len(X), freq="D")
            )
...
class BasicTimeSeriesAnalyzer(TimeSeriesAnalyzer):
        ...
        return {
            ...
            "mann_kendall": {
                ...
                "trend_direction": (
                    "increasing"
                    if mk_statistic > 0
                    else "decreasing" if mk_statistic < 0 else "no_trend"
                ),
            },
        }
        ...
        try:
            ...
            if len(self.series_) >= 24:  # Minimum for decomposition
                ...
                seasonal_strength = float(
                    np.var(decomposition.seasonal) / np.var(self.series_)
                )
                trend_strength = float(
                    np.var(decomposition.trend.dropna()) / np.var(self.series_)
                )
                ...
                return {
                    ...
                    "dominant_pattern": (
                        "seasonal" if seasonal_strength > trend_strength else "trend"
                    ),
                }
            else:
                return {
            ...
            return {"decomposition_available": False, "error": str(e)}
    ...
    def _test_stationarity(self) -> Dict[str, Any]:
        ...
        if kpss_result:
            stationarity_tests["kpss"] = {
                ...
                "critical_values": {
                    str(k): float(v) for k, v in kpss_result[3].items()
                },
    ...
    def _analyze_autocorrelation(self) -> Dict[str, Any]:
        ...
        lb_result = acorr_ljungbox(
            self.series_.dropna(), lags=min(10, len(self.series_) // 4), return_df=True
        )
        ...
        return {
            "autocorrelation_function": {
                ...
                "significant_lags": [
                    i for i, val in enumerate(acf_values) if abs(val) > 0.2
                ],
            },
            "partial_autocorrelation": {
                "values": pacf_values.tolist(),
                "significant_lags": [
                    i for i, val in enumerate(pacf_values) if abs(val) > 0.2
                ],
            },
            "ljung_box_test": {
...
class ARIMAForecaster(TimeSeriesAnalyzer):
    ...
    def fit_forecast(self) -> ForecastResult:
        ...
        if self.auto_arima:
            ...
            self.model_ = ARIMA(
                self.series_, order=order, seasonal_order=seasonal_order
            ).fit()
        ...
        forecast_index = pd.date_range(
            start=last_date, periods=self.forecast_steps + 1, freq=freq
        )[1:]
        ...
        return ForecastResult(
            ...
            model_parameters={
                ...
                "seasonal_order": getattr(
                    self.model_, "seasonal_order", self.seasonal_order
                ),
            },
...
class ExponentialSmoothingForecaster(TimeSeriesAnalyzer):
    ...
    def _get_trend_component(self) -> Optional[str]:
        ...
        mapping = {"simple": None, "double": "add", "triple": "add"}
...
class ChangePointDetector(TimeSeriesAnalyzer):
    ...
    def __init__(self, method: str = "cusum", min_size: int = 10, **kwargs):
    ...
    def detect_change_points(self) -> ChangePointResult:
        ...
        # Convert to dates
        change_point_dates = [
            str(self.series_.index[cp])
            for cp in change_points
            if cp < len(self.series_)
        ]
...
class MultivariateAnalyzer(TimeSeriesAnalyzer):
    ...
    def __init__(self, analysis_type: str = "var", max_lags: int = 5, **kwargs):
    ...
    def analyze_multivariate(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if self.multivariate_analysis_type == "var":
            ...
            raise ValueError(
                f"Unknown multivariate analysis type: {self.multivariate_analysis_type}"
            )
                ...
                if i != j:
                    try:
                        ...
                        if len(test_data) > self.max_lags * 2:
                            ...
                            p_values = []
                            for lag in range(
                                1, min(self.max_lags + 1, len(test_data) // 4 + 1)
                            ):
                            ...
                            results[f"{var2}_causes_{var1}"] = {
                                ...
                                "causality_detected": (
                                    min(p_values) < 0.05 if p_values else False
                                ),
                            }
                    except Exception as e:
                        logger.warning(f"Granger test failed for {var2} -> {var1}: {e}")

        return {"causality_results": results, "variables": variables}


# High-level convenience functions for MCP tool integration


def analyze_time_series_basic(
    ...
    **kwargs
    ...
    try:
        analyzer = BasicTimeSeriesAnalyzer(
            date_column=date_column, value_column=value_column, **kwargs
        )
        analyzer.fit(data)
    ...
    **kwargs
    ...
    try:
        detector = AnomalyDetector(
            date_column=date_column, value_column=value_column, method=method, **kwargs
        )
        detector.fit(data)
        result = detector.detect_anomalies()

        logger.info(
            f"Anomaly detection completed: {len(result.anomaly_indices)} anomalies found"
        )
        return result.to_dict()
...
def detect_change_points(
    ...
    **kwargs
    ...
    try:
        ...
        logger.info(
            f"Change point detection completed: {len(result.change_points)} change points found"
        )
        return result.to_dict()
...
def analyze_multivariate_time_series(
    data: pd.DataFrame, analysis_type: str = "var", **kwargs
) -> Dict[str, Any]:
    """
    Perform multivariate time series analysis.
    ...
    **kwargs
    ...
    try:
        analyzer = MultivariateAnalyzer(analysis_type=analysis_type, **kwargs)
