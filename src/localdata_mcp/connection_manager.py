        ...
        try:
            with self._lock:
                ...
                if health_result.is_healthy:
                    ...
                    logger.warning(
                        f"Database '{name}' initialized but unhealthy: {health_result.error_message}"
                    )
                    return False
    ...
    def close_database(self, name: str) -> bool:
        ...
        try:
            with self._lock:
                ...
                query_ids_to_remove = [
                    qid
                    for qid, start_time in self._query_start_times.items()
    ...
    def _create_enhanced_engine(self, name: str, config: DatabaseConfig) -> Engine:
        ...
        if config.type == DatabaseType.SQLITE:
            engine = create_engine(
                f"sqlite:///{config.connection_string}", **engine_args
            )
            ...
            engine = create_engine(
                f"duckdb:///{config.connection_string}", **engine_args
            )
        elif config.type == DatabaseType.ORACLE:
            from .oracle_support import create_oracle_engine, ORACLEDB_AVAILABLE
            ...
            engine = create_oracle_engine(config.connection_string, auth=auth)
        elif config.type == DatabaseType.MSSQL:
            ...
            auth = config.metadata.get("auth") if config.metadata else None
            engine = create_mssql_engine(config.connection_string, auth=auth)
        else:
    ...
    def _setup_resource_limits(self, name: str, config: DatabaseConfig):
        ...
        limits = {
            ResourceType.MEMORY: ResourceLimit(
                ...
                warning_threshold=perf_config.memory_limit_mb
                * perf_config.memory_warning_threshold,
            ),
            ...
            for limit in limits.values():
                if limit.is_exceeded:
                    ...
                    if limit.resource_type == ResourceType.CONNECTIONS:
                        raise Exception(
                            f"Maximum connections exceeded: {limit.current_value}/{limit.max_value}"
                        )
                    elif limit.resource_type == ResourceType.ERROR_RATE:
                        logger.warning(
                            f"High error rate detected for '{database_name}': {limit.current_value:.1f}%"
                        )
