    ...
    def estimate_processing_requirements(self) -> Dict[str, Any]:
        ...
        try:
            if not OPENPYXL_AVAILABLE:
                raise ValueError(
                    "openpyxl library required for Excel streaming. Install with: pip install openpyxl"
                )
            ...
            if workbook.sheetnames:
                ...
                estimated_memory_per_sheet = (max_row * max_col * 50) / (
                    1024 * 1024
                )  # MB
...
class StreamingCSVProcessor(StreamingFileProcessor):
    ...
    def estimate_processing_requirements(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "estimated_rows": estimated_rows,
                "estimated_memory_mb": file_size_mb
                * 1.5,  # CSV overhead is typically low
                "file_size_mb": file_size_mb,
...
class StreamingCSVSource(StreamingDataSource):
    ...
    def __init__(
        self, file_path: str, progress: Optional[ProcessingProgress] = None, **kwargs
    ):
        self.file_path = file_path
        self.progress = progress
        self.kwargs = kwargs
...
def create_streaming_file_engine(
    ...
    try:
        ...
        for chunk_df in streaming_source.get_chunk_iterator(chunk_size):
            ...
            if chunk_number % 10 == 0:
                logger.info(
                    f"Processed {chunk_number} chunks, {total_rows_processed} total rows"
                )
            ...
            if chunk_number > 1 and chunk_number % 5 == 0:
                ...
                if (
                    processing_time / chunk_number > 2.0
                ):  # More than 2 seconds per chunk
    ...
    logger.info(
        f"Streaming file processing completed: {total_rows_processed} rows in {processing_time:.3f}s, "
