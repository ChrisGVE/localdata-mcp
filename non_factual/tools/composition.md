...
| `cluster_then_chart` | Convenience pipeline: assign_clusters -> render_chart(scatter_fit). Clusters an addressed source, then scatters its first two numeric feature columns coloured by cluster. k pins the cluster count (default: silhouette sweep); format is svg (default) or png. Equivalent to the explicit two-stage compose_pipeline. | DYNAMIC | DYNAMIC | no | `endpoint?`, `path?`, `table?`, `query?`, `k?`, `seed?`, `format?` |
