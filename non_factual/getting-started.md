...
No other tool goes through that gate — the analytical tools, and the regex tools `search_data` and `transform_data`, hand their query to pandas directly, so a statement refused by `execute_query` reaches the driver through `analyze_clusters` or `search_data` alike (issue #25). Treat the query you give any of them as you would any other statement against that database. Setting `security.readonly: true` additionally blocks writes disguised as reads — `SELECT ... INTO`, `CREATE TABLE ... AS SELECT`, `COPY ... TO`, `MERGE INTO` — but only on those two gated tools. Any other tool executes such a statement under the same setting, and a `CREATE TABLE ... AS SELECT` persists (issue #33). Grant the connection read-only rights at the database if that matters.
