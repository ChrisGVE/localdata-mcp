...
## §11 — ClickHouse, evaluated and rejected (2026-07-28)

**ClickHouse is not supported and no code for it remains.** It was built, passed all 18 endpoint
tests, and was then dropped on 2026-07-29. What follows is why, kept because the cost of finding it
out was real and because the next person to reach for ClickHouse should not pay it again.

**The dialect worth using is not the obvious one.** `clickhouse-sqlalchemy` carries a `4 - Beta`
classifier, is sdist-only, has **no GitHub releases at all**, and its PyPI ordering is inverted —
0.2.8 and 0.2.9 were published *after* 0.3.2. The live one is inside ClickHouse's own
`clickhouse-connect`, registered as **`clickhousedb`**, speaking HTTP on 8123; upstream ships a
`MIGRATING_FROM_CLICKHOUSE_SQLALCHEMY.md`. That much is a genuinely useful finding and cost an
afternoon.

**What made it not worth carrying** — four defects, none of them in ClickHouse, all in the driver:

1. **A missing value silently becomes an empty string.** A ClickHouse column is `NOT NULL` unless
   declared `Nullable`, and the portable types render as aliases that inherit that. Inserting `None`
   behaves two different ways by batch size: **one row stores `''` and reports success**
   (`isNull()` = 0); two rows raise `DataError: Invalid None value in non-Nullable column`. A loaded
   file has gaps as a matter of course, so this is the load path rather than an edge of it. Working
   around it meant `Nullable` on every column.
2. **The driver's exceptions are not its DBAPI's exceptions.** `clickhouse_connect.dbapi` exports an
   `Error` as PEP 249 requires, but nothing raised inherits from it, so SQLAlchemy never wraps a
   ClickHouse failure — no `.orig`, not a `SQLAlchemyError`, and invisible to every
   `except SQLAlchemyError` in this codebase. The read-only refusal reached the caller as raw driver
   text instead of the words naming the verb to use instead.
3. **Two Core types cannot be bound.** `LargeBinary` — `dbapi` has no `Binary` constructor, so
   SQLAlchemy's bind processor raises `AttributeError` before any statement is sent. `Time` — the
   column *is* created, but a Python `time` arrives as the literal `14:30:00` where an `Int64` was
   expected. A column that exists and refuses every value is worse than one that does not.
4. **Indexes cannot be reflected.** `CREATE INDEX` without a `TYPE` is refused (code 80); what
   ClickHouse has is a data-skipping index, which is not a lookup; and after creating one,
   `system.data_skipping_indices` shows it while `get_indexes()` and a reflected `Table.indexes`
   both return empty. So `create(type='index')` could not be listed by `info` nor found by `drop`.

Supporting it needed **three seam axes that no other backend used** — one for the mandatory engine
clause on `CREATE TABLE` (ClickHouse has no default table engine, so the DDL fails at *compile*
time with nothing created), one to name driver exceptions SQLAlchemy would not wrap, and one to
refuse the index verb. Three axes carried permanently for one backend, to work around four defects
in somebody else's driver, is the wrong trade.

**What survived the removal**, because it stands on its own:

* `unstorable_column_types()` — it also absorbed Oracle's "no time-of-day type", which had been a
  literal dialect-name branch **in a test fixture**, the one place standing instruction 1 says a
  dialect fact may never live.
* `Backend.open` carrying `read_only_query` onto the read engine for a *server* URL, as `open_file`
  already did for a file. A dialect that can be told read-only in the URL should be told so however
  it was reached.

**The one measurement worth keeping for its own sake:** ClickHouse has no transactions, so the
generic guarantee is not weakened there, it is *absent* — an `INSERT` through a read connection is
simply applied and the row is still there on the next connection. Any future backend without
transactions needs its posture carried in the URL, because there is no floor underneath it.
