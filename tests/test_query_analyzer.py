...
class TestQueryAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with self.engine.connect() as conn:
            ...
            conn.execute(
                ...
                VALUES (:id, :name, :age, :salary, :is_active, :description)
            """),
                test_data,
            )
            conn.commit()
    ...
    def test_query_complexity_analysis(self):
        ...
        test_cases = [
            ...
            (
                "SELECT t1.*, t2.name FROM test_table t1 JOIN test_table t2 ON t1.id = t2.id",
                3,
                {
                    "has_joins": True,
                    "has_aggregations": False,
            ...
            (
                "SELECT COUNT(*), AVG(salary) FROM test_table GROUP BY is_active",
                3,
                {
                    "has_joins": False,
            ...
            # Complex query with subquery (AVG in subquery is still detected as aggregation)
            (
                "SELECT * FROM test_table WHERE salary > (SELECT AVG(salary) FROM test_table)",
                4,
                {
                    "has_joins": False,
                    "has_aggregations": True,
            ...
            # Score: 1 base + 2 join + 2 aggregation + 2 subquery + 2 window = 9
            (
                """SELECT
                    name,
                    salary,
    ...
    def test_sample_row_analysis(self):
        ...
        self.assertEqual(
            column_info["count"], 6
        )  # id, name, age, salary, is_active, description
    ...
    def test_token_estimation(self, mock_tiktoken):
        ...
        column_info = {
            "count": 3,
            "types": {"id": "int64", "name": "object", "description": "object"},
        }
        ...
        # Risk level should be valid
        self.assertIn(
            token_analysis["risk_level"], ["low", "medium", "high", "critical"]
        )
...
10,Jack,27,52000,Marketing"""
    ...
    def test_execute_query_with_analysis(self):
        ...
        result_json = self.db_manager.execute_query(
            "test_csv", query, enable_analysis=True
        )
    ...
    def test_execute_query_without_analysis(self):
        ...
        result_json = self.db_manager.execute_query(
            "test_csv", query, enable_analysis=False
        )
        result = json.loads(result_json)
    ...
    def setUp(self):
        ...
        with self.engine.connect() as conn:
            ...
            for i in range(1, 101):  # 100 rows
                ...
                created_date = (
                    base_date + datetime.timedelta(days=random.randint(0, 365))
                ).isoformat()
                rows.append(
                    {
                        "id": i,
            ...
            conn.execute(
                text("""
                    ...
                    VALUES (:id, :category, :value, :description, :created_date)
                """),
                rows,
            )
            conn.commit()
    ...
    def test_row_count_accuracy(self):
        ...
        for query, expected_rows in test_queries:
            with self.subTest(query=query):
                ...
                if actual_rows > 0:
                    accuracy = (
                        1 - abs(analysis.estimated_rows - actual_rows) / actual_rows
                    )
                    self.assertGreater(
                        accuracy,
                        0.5,
    ...
    def test_memory_estimation_reasonableness(self):
        ...
        for query in queries:
            with self.subTest(query=query):
                ...
                self.assertLess(
                    analysis.estimated_total_memory_mb, 1000
                )  # Shouldn't be crazy high

                # Row size should be reasonable
                self.assertGreater(analysis.estimated_row_size_bytes, 0)
                self.assertLess(
                    analysis.estimated_row_size_bytes, 10000
