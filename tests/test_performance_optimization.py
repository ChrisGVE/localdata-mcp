...
from localdata_mcp.pipeline.integration.performance_optimization import (
    ConversionCache,
    CacheEvictionPolicy,
    CacheStatistics,
    LazyLoadingManager,
...
class MockShimAdapter(ShimAdapter):
    ...
    def estimate_cost(self, request: ConversionRequest):
        from localdata_mcp.pipeline.integration.interfaces import ConversionCost

        return ConversionCost(
            computational_cost=0.1, memory_cost_mb=10.0, time_estimate_seconds=0.1
        )
...
class TestConversionCache:
    ...
    def test_cache_memory_pressure_eviction(self, mock_adapter):
        ...
        cache = ConversionCache(
            max_memory_mb=1.0, eviction_policy=CacheEvictionPolicy.MEMORY_PRESSURE
        )
...
class TestLazyLoadingManager:
    ...
    def test_lazy_loading_manager_initialization(self):
        ...
        manager = LazyLoadingManager(default_threshold_mb=25.0, max_background_tasks=2)
...
class TestPerformanceOptimization:
    ...
    @patch(
        "src.localdata_mcp.pipeline.integration.performance_optimization_complete.PerformanceBenchmark"
    )
    def test_performance_benchmark_creation(self, mock_benchmark_class):
        """Test performance benchmark creation and usage."""
        from localdata_mcp.pipeline.integration.performance_optimization_complete import (
        ...
        optimizer_suite = create_performance_optimizer(
            cache_size=100, memory_limit_mb=50.0
        )
...
class TestIntegrationPoints:
    ...
    def test_optimization_suite_creation(self):
        ...
        suite = create_performance_optimizer(
            cache_size=500, memory_limit_mb=200.0, lazy_threshold_mb=25.0
        )
    ...
    @patch(
        "src.localdata_mcp.pipeline.integration.performance_optimization_complete.OptimizationSelector"
    )
    def test_request_optimization(self, mock_selector_class, sample_request):
        """Test automatic request optimization."""
        from localdata_mcp.pipeline.integration.performance_optimization_complete import (
            optimize_conversion_request,
            create_performance_optimizer,
        )
        ...
        mock_selector.analyze_data_characteristics.return_value = Mock(
            data_size_mb=10.0, complexity_score=0.5, estimated_processing_time=1.0
        )
