...
from localdata_mcp.pipeline.integration.schema_validation import (
...
from localdata_mcp.pipeline.integration.interfaces import DataFormat
...
class TestSchemaInferenceEngine:
    ...
    def test_dataframe_schema_inference(self, inference_engine, sample_dataframe):
        ...
        result = inference_engine.infer_schema(
            sample_dataframe, DataFormat.PANDAS_DATAFRAME
        )
    ...
    def test_numpy_schema_inference(self, inference_engine, sample_numpy_array):
        ...
        result = inference_engine.infer_schema(
            sample_numpy_array, DataFormat.NUMPY_ARRAY
        )
    ...
    def test_sparse_matrix_inference(self, inference_engine, sample_sparse_matrix):
        ...
        result = inference_engine.infer_schema(
            sample_sparse_matrix, DataFormat.SCIPY_SPARSE
        )
    ...
    def test_low_confidence_warning(self, inference_engine):
        ...
        mixed_data = pd.DataFrame({"mixed_col": [1, "two", 3.0, None, True, [1, 2, 3]]})
    ...
    def test_alternative_schemas(self, inference_engine):
        ...
        ambiguous_data = pd.DataFrame({"numbers_as_strings": ["1", "2", "3", "4", "5"]})
...
class TestSchemaValidator:
    ...
    def test_type_validation_errors(self, validator, valid_dataframe_schema):
        ...
        type_errors = [
            e for e in result.errors if e.error_type == ValidationRuleType.TYPE_CHECK
        ]
    ...
    def test_missing_column_validation(self, validator, valid_dataframe_schema):
        ...
        missing_column_errors = [
            e for e in result.errors if "missing" in e.error_message.lower()
        ]
    ...
    def test_extra_column_handling(self, validator):
        ...
        extra_column_warnings = [
            w for w in result.warnings if "extra" in w.error_message.lower()
        ]
    ...
    def test_custom_validation_rule(self, validator, valid_dataframe_schema):
        ...
        class CustomEmailValidation(ValidationRule):
            def __init__(self):
                super().__init__(
                    "email_validation",
                    ValidationRuleType.PATTERN_CHECK,
        ...
        custom_errors = [
            e for e in result.errors if e.error_type == ValidationRuleType.PATTERN_CHECK
        ]
    ...
    def test_max_errors_limit(self, validator, valid_dataframe_schema):
        ...
        result = validator.validate_data(
            bad_data, valid_dataframe_schema, max_errors=10
        )
...
class TestSchemaEvolutionManager:
    ...
    def test_schema_version_management(
        self, evolution_manager, base_schema, evolved_schema
    ):
    ...
    def test_compatibility_score_calculation(
        ...
        score = evolution_manager.calculate_compatibility_score(
            base_schema, evolved_schema
        )
        ...
        identical_score = evolution_manager.calculate_compatibility_score(
    ...
    def test_recommendations_generation(self, evolution_manager, base_schema):
        ...
        assert any(
            keyword in rec_text
            for keyword in ["field", "column", "migration", "transform"]
        )

    def test_compatibility_caching(
        self, evolution_manager, base_schema, evolved_schema
    ):
        """Test that compatibility scores are cached for performance."""
        # Calculate score twice
        score1 = evolution_manager.calculate_compatibility_score(
            base_schema, evolved_schema
        )
        score2 = evolution_manager.calculate_compatibility_score(
            base_schema, evolved_schema
        )
    ...
    def test_drift_level_categorization(self, evolution_manager):
        ...
        minimal_score = evolution_manager.calculate_compatibility_score(
            base, minimal_drift
        )
