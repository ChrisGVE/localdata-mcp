...
class TestAnalyticalFormats:
    ...
    def test_parquet_file_connection(self, manager, sample_dataframe):
        ...
        try:
            ...
            query_result = manager.execute_query(
                "parquet_test", "SELECT COUNT(*) as total FROM data_table"
            )
            ...
            query_result = manager.execute_query(
                "parquet_test",
                "SELECT category, COUNT(*) as count FROM data_table GROUP BY category",
            )
    ...
    def test_feather_file_connection(self, manager, sample_dataframe):
        ...
        try:
            ...
            # Test querying the data
            query_result = manager.execute_query(
                "feather_test", "SELECT COUNT(*) as total FROM data_table"
            )
            assert "1000" in query_result
            ...
            query_result = manager.execute_query(
                "feather_test",
                "SELECT COUNT(*) as count FROM data_table WHERE timestamp >= '2023-01-01'",
            )
            assert "1000" in query_result
    ...
    def test_arrow_file_connection(self, manager, sample_dataframe):
        ...
        try:
            ...
            # Test querying the data
            query_result = manager.execute_query(
                "arrow_test", "SELECT COUNT(*) as total FROM data_table"
            )
            assert "1000" in query_result

            # Test aggregation query
            query_result = manager.execute_query(
                "arrow_test", "SELECT AVG(value) as avg_value FROM data_table"
            )
    ...
    def test_pyarrow_missing_parquet(self, manager):
        ...
        with patch("localdata_mcp.localdata_mcp.PYARROW_AVAILABLE", False):
            with tempfile.NamedTemporaryFile(suffix=".csv") as temp_file:
                ...
                with pytest.raises(
                    ValueError, match="pyarrow library is required for Parquet"
                ):
    ...
    def test_pyarrow_missing_feather(self, manager):
        ...
        with patch("localdata_mcp.localdata_mcp.PYARROW_AVAILABLE", False):
            with tempfile.NamedTemporaryFile(suffix=".csv") as temp_file:
                ...
                with pytest.raises(
                    ValueError, match="pyarrow library is required for Feather"
                ):
    ...
    def test_pyarrow_missing_arrow(self, manager):
        ...
        with patch("localdata_mcp.localdata_mcp.PYARROW_AVAILABLE", False):
            with tempfile.NamedTemporaryFile(suffix=".csv") as temp_file:
                ...
                with pytest.raises(
                    ValueError, match="pyarrow library is required for Arrow"
                ):
    ...
    def test_large_analytical_file_handling(self, manager):
        ...
        try:
            ...
            is_large = manager._is_large_file(
                temp_path, threshold_mb=1
            )  # 1MB threshold
            ...
            query_result = manager.execute_query(
                "large_parquet", "SELECT COUNT(*) as total FROM data_table"
            )
    ...
    def test_analytical_format_data_types(self, manager):
        ...
        try:
            ...
            query_result = manager.execute_query(
                "typed_parquet", "SELECT AVG(float_col) as avg_float FROM data_table"
            )
            ...
            query_result = manager.execute_query(
                "typed_parquet",
                "SELECT COUNT(*) as true_count FROM data_table WHERE bool_col = 1",
            )
    ...
    def test_analytical_format_performance(self, manager, sample_dataframe):
        ...
        try:
            ...
            start_time = time.time()
            query_result = manager.execute_query(
                "perf_parquet", "SELECT COUNT(*) FROM data_table"
            )
            ...
            start_time = time.time()
            query_result = manager.execute_query(
                "perf_feather", "SELECT COUNT(*) FROM data_table"
            )
    ...
    def test_corrupted_analytical_files(self, manager):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".parquet", delete=False, mode="w"
        ) as temp_file:
...
class TestAnalyticalFormatIntegration:
    ...
    def test_describe_database_analytical(self, manager):
        ...
        test_df = pd.DataFrame(
            {"id": [1, 2, 3], "name": ["A", "B", "C"], "value": [10.5, 20.7, 30.2]}
        )
    ...
    def test_query_buffering_analytical(self, manager):
        ...
        large_df = pd.DataFrame(
            {"id": range(150), "data": [f"item_{i}" for i in range(150)]}
        )
        ...
        try:
            ...
            result = manager.execute_query("buffer_test", "SELECT * FROM data_table")
