...
class TestQueryTimeoutManager(unittest.TestCase):
    ...
    def test_get_timeout_config_existing_database(self):
        ...
        )  # SQLite supports limited cancellation
    ...
    def test_timeout_context_successful_execution(self):
        ...
        with self.timeout_manager.timeout_context(
            operation_id, timeout_config, cleanup_func
        ) as context:
    ...
    def test_manual_cancellation(self):
        ...
        with self.timeout_manager.timeout_context(
            ...
            # Manually cancel it
            success = self.timeout_manager.cancel_operation(
                operation_id, TimeoutReason.MANUAL_CANCEL
            )
...
class TestTimeoutIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cursor.executemany(
            "INSERT INTO test_data (id, name, value) VALUES (?, ?, ?)", test_data
        )
...
class TestTimeoutConfiguration(unittest.TestCase):
    ...
    def test_per_database_timeout_configuration(self):
        ...
        with patch(
            ...
            mock_config_manager.get_database_config.side_effect = (
                lambda name: configs.get(name)
            )
            mock_get_config.return_value = mock_config_manager
