...
class TestTimeoutSystemIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(1500):
            test_data.append(
                (i, f"test_data_{i}_with_long_content_to_simulate_real_data", i * 1.5)
            )

        cursor.executemany(
            "INSERT INTO large_dataset (id, data, value) VALUES (?, ?, ?)", test_data
        )
        ...
        cursor.executemany(
            "INSERT INTO slow_query_test (id, complex_data) VALUES (?, ?)", complex_data
        )
    ...
    def test_timeout_manager_active_operations(self):
        ...
        result = self.mcp_server.execute_query(
            "test_fast_db", "SELECT COUNT(*) FROM large_dataset"
        )
