...
def create_mock_excel_file(
    file_path: str, sheets: Optional[Dict[str, pd.DataFrame]] = None
):
...
def mock_external_dependencies():
    ...
    with (
        ...
        def mock_resolve_func():
            ...
            return (
                mock_resolve_func.last_path
                if hasattr(mock_resolve_func, "last_path")
                else Path("/tmp/test.csv")
            )
...
def sample_json_data():
    ...
    return [{"id": i, "name": f"item_{i}", "value": i * 10} for i in range(1, 21)]
...
def mock_excel_workbook():
    ...
    sheet1_data = pd.DataFrame(
        {"A": [1, 2, 3, 4], "B": ["a", "b", "c", "d"], "C": [10.1, 20.2, 30.3, 40.4]}
    )

    sheet2_data = pd.DataFrame(
        {"metric": ["cpu", "memory", "disk"], "value": [75.5, 82.1, 45.8]}
    )
    ...
    with (
        ...
        mock_excel_file.return_value.__enter__.return_value.sheet_names = (
...
def mock_file_operations():
    ...
    def mock_file_reader(file_path, file_type="csv"):
        ...
        if file_type == "csv" or file_path.endswith(".csv"):
            ...
            return pd.DataFrame({"col1": ["val1", "val2"], "col2": ["val3", "val4"]})
    ...
    with (
        ...
        mock_read_csv.side_effect = lambda *args, **kwargs: mock_file_reader(
            args[0] if args else "test.csv", "csv"
        )
        mock_read_json.side_effect = lambda *args, **kwargs: mock_file_reader(
            args[0] if args else "test.json", "json"
        )
        mock_read_parquet.side_effect = lambda *args, **kwargs: mock_file_reader(
            args[0] if args else "test.parquet", "parquet"
        )
        mock_read_feather.side_effect = lambda *args, **kwargs: mock_file_reader(
            args[0] if args else "test.feather", "feather"
        )
...
def mock_config_parser():
    ...
    def mock_getitem(section_name):
        if section_name == "database":
            return {"host": "localhost", "port": "5432", "name": "testdb"}
        elif section_name == "logging":
            return {"level": "INFO", "file": "app.log"}
        return {}
...
def mock_corrupted_files():
    ...
    with (
        patch("pandas.read_csv", side_effect=raise_error),
