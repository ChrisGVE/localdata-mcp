...
class TestSpreadsheetFormats:
    ...
    def test_excel_file_missing_library(self, manager, test_data_dir):
        ...
        with patch("localdata_mcp.localdata_mcp.OPENPYXL_AVAILABLE", False):
            with pytest.raises(ValueError, match="openpyxl library is required"):
                # Use .xlsx extension so the method hits the openpyxl check
                manager._load_excel_file("/tmp/dummy_file.xlsx")
    ...
    def test_ods_file_missing_library(self, manager, test_data_dir):
        ...
        with patch("localdata_mcp.localdata_mcp.ODFPY_AVAILABLE", False):
            with pytest.raises(ValueError, match="odfpy library is required"):
                ...
                manager._load_ods_file(
                    str(test_data_dir / "test_data.csv")
                )  # Using CSV as placeholder
    ...
    def test_file_size_detection(self, manager, test_data_dir):
        ...
        # File is 274 bytes, so threshold must be below that in MB
        is_large = manager._is_large_file(
            str(csv_file), threshold_mb=0.0001
        )  # Threshold ~105 bytes, below file size
    ...
    def test_empty_file_handling(self, manager, test_data_dir):
        ...
        if "Successfully connected" in result:
            ...
            query_result = manager.execute_query(
                "empty_csv", "SELECT COUNT(*) as row_count FROM data_table"
            )
    ...
    def test_corrupted_file_handling(self, manager, test_data_dir):
        ...
        result = manager.connect_database(
            "corrupted_excel", "excel", str(corrupted_file)
        )
...
class TestDataTypeHandling:
    ...
    def test_date_handling(self, manager):
        ...
        try:
            ...
            query_result = manager.execute_query(
                "date_test", "SELECT * FROM data_table WHERE date_col >= '2023-01-01'"
            )
    ...
    def test_numeric_handling(self, manager):
        ...
        try:
            ...
            query_result = manager.execute_query(
                "numeric_test", "SELECT AVG(float_col) as avg_price FROM data_table"
            )
            assert (
                "24.99" in query_result or "25" in query_result
            )  # Average of 19.99 and 29.99
    ...
    def test_boolean_handling(self, manager):
        ...
        try:
            ...
            query_result = manager.execute_query(
                "boolean_test",
                "SELECT * FROM data_table WHERE active = 'True' OR active = '1'",
            )
