...
from localdata_mcp.streaming_executor import MemoryStatus, StreamingQueryExecutor
...
class TestStreamingActivationLogic:
    ...
    def test_streaming_not_activated_small_dataset(self):
        ...
        pipeline = StreamingDataPipeline(
            ...
            streaming_threshold_mb=1000,
        )  # High threshold

        with patch(
            "src.localdata_mcp.pipeline.core.get_logging_manager"
        ) as mock_logging:
    ...
    def test_streaming_activated_by_size_threshold(self):
        ...
        pipeline = StreamingDataPipeline(
            [
                ...
                ),  # Use SGD for partial_fit support
            ],
            streaming_threshold_mb=0.01,
        )  # Very low threshold to trigger streaming
    ...
    def test_streaming_activated_by_memory_pressure(self):
        ...
        with patch(
            ...
            with patch(
                "src.localdata_mcp.pipeline.core.StreamingQueryExecutor"
...
class TestStreamingPipelineExecution:
    ...
    def test_streaming_transform_execution(self, mock_get_logging):
        ...
        X_train, y_train = make_classification(
            n_samples=500, n_features=8, random_state=42
        )
        X_train_df = pd.DataFrame(X_train, columns=[f"feature_{i}" for i in range(8)])

        X_test = pd.DataFrame(
            np.random.random((800, 8)), columns=[f"feature_{i}" for i in range(8)]
        )
    ...
    def test_chunk_aggregation_dataframe(self):
        ...
        pipeline = StreamingDataPipeline([("scaler", StandardScaler())])
...
class TestAdaptiveChunkSizing:
    ...
    def test_adaptive_chunk_size_calculation(self):
        ...
        pipeline = StreamingDataPipeline(
            [("scaler", StandardScaler())], adaptive_chunking=True
        )
...
class TestStreamingErrorHandling:
    ...
    def test_streaming_fit_error_handling(self, mock_get_logging):
        ...
        pipeline = StreamingDataPipeline(
            [("scaler", StandardScaler()), ("classifier", mock_classifier)],
            streaming_threshold_mb=0.01,
        )  # Force streaming
    ...
    def test_streaming_transform_error_handling(self, mock_get_logging):
        ...
        with patch(
            ...
            with patch.object(
                pipeline,
                "_transform_chunk",
                side_effect=RuntimeError("Transform chunk failed"),
            ):
                with pytest.raises(PipelineError) as excinfo:
...
class TestStreamingMetadata:
    ...
    def test_streaming_metadata_generation(self):
        ...
        pipeline._chunk_transform_results = [
            pd.DataFrame({"a": [1, 2]}),
            pd.DataFrame({"a": [3, 4]}),
        ]
...
class TestStreamingPerformanceValidation:
    ...
    def test_memory_efficiency_large_dataset(self):
        ...
        assert (
            memory_efficiency["final_memory_gb"] >= 3.5
        )  # Still have available memory
