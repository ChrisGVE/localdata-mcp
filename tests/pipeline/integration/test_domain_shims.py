...
class TestBaseDomainShim:
    ...
    def test_extract_domain_from_format(self):
        ...
        assert (
            self.test_shim._extract_domain_from_format(DataFormat.STATISTICAL_RESULT)
            == "statistical"
        )

        # Test regression formats
        assert (
            self.test_shim._extract_domain_from_format(DataFormat.REGRESSION_MODEL)
            == "regression"
        )

        # Test time series formats
        assert (
            self.test_shim._extract_domain_from_format(DataFormat.TIME_SERIES)
            == "time_series"
        )
        assert (
            self.test_shim._extract_domain_from_format(DataFormat.FORECAST_RESULT)
            == "time_series"
        )

        # Test pattern recognition formats
        assert (
            self.test_shim._extract_domain_from_format(DataFormat.CLUSTERING_RESULT)
            == "pattern_recognition"
        )
        assert (
            self.test_shim._extract_domain_from_format(
        ...
        assert (
            self.test_shim._extract_domain_from_format(DataFormat.PANDAS_DATAFRAME)
            == "generic"
        )
...
class TestRegressionShim:
    ...
    def test_convert_feature_importance_to_pattern_recognition(self):
        ...
        assert result.converted_data["feature_names"] == [
            "feat1",
            "feat2",
            "feat3",
            "feat4",
    ...
    def test_convert_model_summary_to_statistical(self):
        ...
        source_data = {"coefficients": [1.2, -0.8, 0.5], "p_values": [0.01, 0.05, 0.12]}
    ...
    def test_normality_test_implementation(self):
        ...
        non_normal_residuals = np.concatenate([np.ones(50), np.zeros(50)])
...
class TestPatternRecognitionShim:
    ...
    def test_convert_classification_results_to_statistical(self):
        ...
        source_data = {"predictions": predictions, "probabilities": probabilities}
    ...
    def test_convert_cluster_labels_to_regression(self):
        ...
        source_data = {"cluster_labels": np.array([0, 1, 0, 2, 1, 2, 0])}
...
class TestDomainShimIntegration:
    ...
    def test_statistical_to_regression_to_time_series_workflow(self):
        ...
        correlation_matrix = pd.DataFrame(
            {"A": [1.0, 0.7, -0.3], "B": [0.7, 1.0, 0.2], "C": [-0.3, 0.2, 1.0]},
            index=["A", "B", "C"],
        )
