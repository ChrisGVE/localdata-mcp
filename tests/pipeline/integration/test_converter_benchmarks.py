...
class TestPandasConverterBenchmarks:
        ...
        datasets["mixed"] = pd.DataFrame(
            {
                ...
                **{
                    f"bool_{i}": np.random.choice([True, False], 10000)
                    for i in range(3)
                },
            }
        )
        ...
        for name, df in self.datasets.items():
            if name != "mixed":  # Skip mixed for numeric conversion
                request = create_conversion_request(
                    df, DataFormat.PANDAS_DATAFRAME, DataFormat.NUMPY_ARRAY
                )
    ...
    def test_dataframe_to_sparse_performance(self):
        ...
        for name in sparse_datasets:
            if name in self.datasets:
                ...
                request = create_conversion_request(
                    df, DataFormat.PANDAS_DATAFRAME, DataFormat.SCIPY_SPARSE
                )
    ...
    def test_mixed_types_handling_performance(self):
        ...
        assert (
            benchmark.conversion_time < 15.0
        )  # Reasonable time even with type handling
    ...
    def test_memory_efficient_options_performance(self):
        ...
        standard_benchmark = self.profiler.profile_conversion(
            self.converter, standard_request
        )
        ...
        efficient_benchmark = self.profiler.profile_conversion(
            memory_efficient_converter, efficient_request
        )
        ...
        memory_diff_standard = (
            standard_benchmark.memory_peak - standard_benchmark.memory_before
        )
        memory_diff_efficient = (
            efficient_benchmark.memory_peak - efficient_benchmark.memory_before
        )
    ...
    def _print_benchmark_results(self, results: List[BenchmarkResult], test_name: str):
        ...
        print(
            "Dataset        | Size           | Time(s) | Memory(MB) | Quality | Warnings"
        )
        print("-" * 75)
        ...
        for result in results:
            size_str = (
                ...
                if len(result.data_size) > 1
                else str(result.data_size[0])
            )
            memory_change = result.memory_after - result.memory_before

            print(
                f"{result.test_name.split('_')[1]:12} | {size_str:12} | {result.conversion_time:6.3f} | {memory_change:8.1f} | {result.quality_score:6.3f} | {result.warnings_count:8}"
            )
...
class TestNumpyConverterBenchmarks:
    ...
    def test_numpy_to_dataframe_performance(self):
        ...
        for name in test_arrays:
            ...
            request = create_conversion_request(
                array, DataFormat.NUMPY_ARRAY, DataFormat.PANDAS_DATAFRAME
            )
    ...
    def test_numpy_to_sparse_performance(self):
        ...
        for name in sparse_arrays:
            array = self.arrays[name]
            request = create_conversion_request(
                array, DataFormat.NUMPY_ARRAY, DataFormat.SCIPY_SPARSE
            )
    ...
    def test_dtype_preservation_performance(self):
        ...
        for dtype_name in dtypes_to_test:
            ...
            request = create_conversion_request(
                array, DataFormat.NUMPY_ARRAY, DataFormat.PYTHON_DICT
            )
    ...
    def _print_benchmark_results(self, results: List[BenchmarkResult], test_name: str):
        ...
        for result in results:
            print(
                f"{result.test_name}: {result.conversion_time:.3f}s, Quality: {result.quality_score:.3f}"
            )
...
class TestSparseMatrixBenchmarks:
        ...
        for target_format in dense_targets:
            for name, sparse_matrix in self.sparse_matrices.items():
                if "large" not in name:  # Skip very large matrices for dense conversion
                    request = create_conversion_request(
                        sparse_matrix, DataFormat.SCIPY_SPARSE, target_format
                    )

                    benchmark = self.profiler.profile_conversion(
                        self.converter, request
                    )
                    results.append(benchmark)
    ...
    def test_sparse_format_conversion_performance(self):
        ...
        csr_to_dict_request = create_conversion_request(
            csr_matrix, DataFormat.SCIPY_SPARSE, DataFormat.PYTHON_DICT
        )

        dict_benchmark = self.profiler.profile_conversion(
            self.converter, csr_to_dict_request
        )
    ...
    def test_very_sparse_matrix_efficiency(self):
        ...
        print(
            f"Testing very sparse matrix: {very_sparse.shape}, density: {density:.6f}"
        )
    ...
    def test_dense_to_sparse_conversion_efficiency(self):
        ...
        sparse_patterns = {
            "very_sparse": (np.random.rand(1000, 500) > 0.99).astype(
                float
            ),  # 1% non-zero
            "moderately_sparse": (np.random.rand(500, 200) > 0.7).astype(
                float
            ),  # 30% non-zero
        ...
        for pattern_name, array in sparse_patterns.items():
            ...
            print(
                f"{pattern_name} to sparse: {benchmark.conversion_time:.3f}s, "
                f"warnings: {benchmark.warnings_count}"
            )

    def _print_sparse_benchmark_results(
        self, results: List[BenchmarkResult], test_name: str
    ):
        """Print sparse benchmark results."""
        print(f"\n=== {test_name} Benchmark Results ===")
        for result in results:
            print(
                f"{result.test_name}: {result.conversion_time:.3f}s, "
...
class TestMemoryEfficiencyComparison:
    ...
    def test_memory_efficiency_comparison(self):
        ...
        for name, result in results.items():
            ...
            print(
                f"{name:15} | {result.conversion_time:6.3f} | {memory_peak:12.1f} | {result.quality_score:6.3f}"
            )

        # Assertions for memory efficiency
        # Memory efficient should use less peak memory
        standard_peak = (
            results["standard"].memory_peak - results["standard"].memory_before
        )
        efficient_peak = (
            results["memory_efficient"].memory_peak
            - results["memory_efficient"].memory_before
        )
...
if __name__ == "__main__":
    ...
    pytest.main(
        [
            __file__
            + "::TestPandasConverterBenchmarks::test_dataframe_to_numpy_performance_scaling",
            "-v",
            "-s",
        ]
    )
