...
class TestMetadataLineage:
    ...
    def test_lineage_creation(self):
        ...
        lineage = MetadataLineage(
            original_source="test_source", current_format=DataFormat.PANDAS_DATAFRAME
        )
...
class TestPandasMetadataTransformer:
    ...
    def test_can_transform_pandas_formats(self):
        ...
        # DataFrame to array
        assert (
            self.transformer.can_transform(
    ...
    def test_transform_dataframe_to_array(self):
        ...
        transformed = self.transformer.transform_metadata(
            original_metadata, DataFormat.PANDAS_DATAFRAME, DataFormat.NUMPY_ARRAY
        )
    ...
    def test_transform_array_to_dataframe(self):
        ...
        transformed = self.transformer.transform_metadata(
            original_metadata, DataFormat.NUMPY_ARRAY, DataFormat.PANDAS_DATAFRAME
        )
    ...
    def test_transform_to_time_series(self):
        ...
        original_metadata = {"data_format": "pandas_dataframe", "shape": (100, 2)}

        transformed = self.transformer.transform_metadata(
            original_metadata, DataFormat.PANDAS_DATAFRAME, DataFormat.TIME_SERIES
        )
...
class TestNumpyMetadataTransformer:
    ...
    def test_can_transform_numpy_formats(self):
        ...
        assert self.transformer.can_transform(DataFormat.JSON, DataFormat.CSV) is False
...
class TestMetadataManager:
    ...
    def test_manager_initialization(self):
        ...
        assert (
            len(self.manager._transformers) >= 2
        )  # At least pandas and numpy transformers
        assert (
            len(self.manager._metadata_schemas) >= 2
        )  # At least pandas and numpy schemas

    def test_extract_dataframe_metadata(self):
        """Test extracting metadata from DataFrame."""
        metadata = self.manager.extract_metadata(
            self.sample_df, DataFormat.PANDAS_DATAFRAME
        )
    ...
    def test_extract_numpy_metadata(self):
        ...
        metadata = self.manager.extract_metadata(
            self.sample_array, DataFormat.NUMPY_ARRAY
        )
    ...
    def test_apply_metadata_to_data(self):
        ...
        metadata = {"data_format": "numpy_array", "shape": (3, 2), "dtype": "int64"}

        enhanced_data = self.manager.apply_metadata(
            self.sample_array, metadata, DataFormat.NUMPY_ARRAY
        )
    ...
    def test_merge_metadata_without_conflicts(self):
        ...
        metadata1 = {"source": "test1", "timestamp": "2023-01-01", "shape": (10, 2)}
    ...
    def test_merge_metadata_with_conflicts(self):
        ...
        metadata1 = {"version": "1.0", "timestamp": "2023-01-01T10:00:00"}
    ...
    def test_transform_metadata_with_lineage(self):
        ...
        self.manager._lineage_tracking["test_lineage"] = MetadataLineage(
            original_source="test", current_format=DataFormat.PANDAS_DATAFRAME
        )
    ...
    def test_validate_metadata_no_schema(self):
        ...
        result = self.manager.validate_metadata(
    ...
    def test_preservation_rules_application(self):
        ...
        transformed = self.manager._apply_preservation_rules(
            metadata, DataFormat.PANDAS_DATAFRAME, DataFormat.NUMPY_ARRAY
        )
...
class TestMetadataConflictResolution:
    ...
    def test_numeric_conflict_resolution(self):
        ...
        resolved = self.manager._resolve_merge_conflict("score", 10.0, 20.0)
    ...
    def test_default_conflict_resolution(self):
        ...
        resolved = self.manager._resolve_merge_conflict(
            "unknown_key", "old_value", "new_value"
        )
