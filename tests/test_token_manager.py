...
class TestTokenManager:
    ...
    def test_estimate_text_column_tokens(self):
        ...
        result = self.token_manager._estimate_text_column_tokens(
    ...
    def test_estimate_other_column_tokens(self):
        ...
        datetime_tokens = self.token_manager._estimate_other_column_tokens(
            datetime_series
        )
    ...
    def test_estimate_tokens_from_dataframe_large(self):
        ...
        large_df = pd.DataFrame(
            {
                "id": range(200),
                "description": [
                    f"This is item number {i} with some text" for i in range(200)
                ],
            }
        )
        ...
        assert (
            estimation.sample_size == self.token_manager.sample_size
        )  # Should use sample
    ...
    def test_get_response_metadata(self):
        ...
        df = pd.DataFrame(
            {"numeric_col": range(1000), "text_col": ["some text content"] * 1000}
        )
        ...
        assert all(
            "fits_in_context" in info for info in metadata.model_compatibility.values()
        )
    ...
    def test_chunking_recommendations(self):
        ...
        if large_metadata.chunking_recommendation:
            ...
            assert large_metadata.chunking_recommendation.strategy in [
                "row_based",
                "column_based",
                "mixed",
            ]
    ...
    def test_confidence_calculation(self):
        ...
        assert (
            small_est.confidence > large_est.confidence
        )  # Sampling reduces confidence
    ...
    def test_performance_estimation(self):
        ...
        complex_df = pd.DataFrame(
            {
                "long_text": ["This is a very long text field with lots of content"]
                * 1000
            }
        )
        ...
        assert (
            complex_meta.estimated_response_time >= simple_meta.estimated_response_time
        )
...
class TestTokenManagerIntegration:
    ...
    def test_streaming_executor_metadata_format(self):
        ...
        df = pd.DataFrame({"id": range(100), "description": ["Sample text"] * 100})
...
class TestTokenEstimationAccuracy:
    ...
    def test_numeric_column_accuracy(self):
        ...
        assert (
            abs(estimation.tokens_per_row - (expected_content_tokens + json_overhead))
            < 1
        )
        ...
        df = pd.DataFrame(
            {
                ...
                "name": (["John", "Jane", "Bob"] * 16)
                + ["John", "Jane"],  # Text: variable (50 total)
                "active": [True, False] * 25,  # Boolean: 1 token each
                "created": pd.date_range(
                    "2023-01-01", periods=50
                ),  # Datetime: ~3 tokens
