...
from localdata_mcp.pipeline.integration.pipeline_analyzer import (
    PipelineAnalyzer,
    ShimInjector,
    PipelineValidator,
    PipelineStep,
...
from localdata_mcp.pipeline.integration.shim_registry import (
    ShimRegistry,
    EnhancedShimAdapter,
)
...
def mock_shim_registry():
    ...
    adapter2.estimate_cost.return_value = ConversionCost(
        computational_cost=0.3, memory_cost_mb=80, time_estimate_seconds=1.5
    )
...
class TestPipelineAnalyzer:
    ...
    def test_analyzer_initialization(
        self, mock_compatibility_matrix, mock_shim_registry
    ):
    ...
    def test_build_pipeline_connections(
        self, mock_compatibility_matrix, mock_shim_registry, sample_pipeline_steps
    ):
    ...
    def test_analyze_compatibility_compatible_pipeline(
        self, mock_compatibility_matrix, mock_shim_registry, sample_pipeline_steps
    ):
    ...
    def test_analyze_compatibility_incompatible_pipeline(
        self, mock_compatibility_matrix, mock_shim_registry, incompatible_pipeline_steps
    ):
    ...
    def test_analyze_pipeline_complete(
        self, mock_compatibility_matrix, mock_shim_registry, sample_pipeline_steps
    ):
    ...
    def test_analyze_pipeline_with_caching(
        self, mock_compatibility_matrix, mock_shim_registry, sample_pipeline_steps
    ):
        """Test pipeline analysis with caching."""
        analyzer = PipelineAnalyzer(
            mock_compatibility_matrix, mock_shim_registry, enable_caching=True
        )

        # First analysis
        result1 = analyzer.analyze_pipeline(
            sample_pipeline_steps, pipeline_id="cached_test"
        )

        # Second analysis (should use cache)
        result2 = analyzer.analyze_pipeline(
            sample_pipeline_steps, pipeline_id="cached_test"
        )
    ...
    def test_generate_shim_recommendations(
        self, mock_compatibility_matrix, mock_shim_registry, incompatible_pipeline_steps
    ):
...
class TestShimInjector:
    ...
    def test_injector_initialization(
        self, mock_shim_registry, mock_compatibility_matrix
    ):
    ...
    def test_select_recommendations_by_strategy(
        self, mock_shim_registry, mock_compatibility_matrix
    ):
        ...
        rec1.confidence = 0.9
        rec1.cost_estimate = ConversionCost(
            computational_cost=0.2, memory_cost_mb=50, time_estimate_seconds=1.0
        )

        rec2 = Mock(spec=ShimRecommendation)
        rec2.confidence = 0.5
        rec2.cost_estimate = ConversionCost(
            computational_cost=0.9, memory_cost_mb=200, time_estimate_seconds=5.0
        )

        recommendations = [rec1, rec2]

        # Test BALANCED strategy
        selected = injector._select_recommendations_by_strategy(
            recommendations, InjectionStrategy.BALANCED
        )
        assert (
            len(selected) == 1
        )  # Only rec1 should pass (confidence > 0.6 and cost < 0.8)
        assert selected[0] == rec1

        # Test SAFE strategy
        selected_safe = injector._select_recommendations_by_strategy(
            recommendations, InjectionStrategy.SAFE
        )
        assert len(selected_safe) == 2  # All recommendations

    def test_optimize_shim_selection(
        self, mock_shim_registry, mock_compatibility_matrix
    ):
        ...
        selected_adapter, score = injector.optimize_shim_selection(
            compatible_adapters, request
        )
    ...
    def test_inject_single_shim(self, mock_shim_registry, mock_compatibility_matrix):
        ...
        modified_steps, injection_info = injector._inject_single_shim(
...
class TestPipelineValidator:
    ...
    def test_validate_pipeline_structure(
        ...
        validator = PipelineValidator(
            mock_compatibility_matrix, mock_shim_registry, mock_analyzer, mock_injector
        )
    ...
    def test_validate_and_fix_pipeline(
        self,
        mock_compatibility_matrix,
        mock_shim_registry,
        mock_analyzer,
...
class TestFactoryFunctions:
    ...
    def test_create_pipeline_analyzer(
        self, mock_compatibility_matrix, mock_shim_registry
    ):
        """Test analyzer factory function."""
        analyzer = create_pipeline_analyzer(
            mock_compatibility_matrix, mock_shim_registry
        )
    ...
    def test_create_pipeline_validator(
        self, mock_compatibility_matrix, mock_shim_registry
    ):
    ...
    def test_create_optimization_criteria(self):
        ...
        criteria = create_optimization_criteria(
            prioritize_performance=False, quality_threshold=0.9
        )
...
class TestUtilityFunctions:
    ...
    def test_analyze_and_fix_pipeline(
        self, mock_compatibility_matrix, mock_shim_registry, sample_pipeline_steps
    ):
        """Test high-level analyze and fix utility."""
        with patch(
            "src.localdata_mcp.pipeline.integration.pipeline_analyzer.create_pipeline_validator"
...
class TestErrorHandling:
    ...
    def test_analyzer_error_handling(
        ...
        mock_compatibility_matrix.get_compatibility.side_effect = Exception(
            "Test error"
        )
    ...
    def test_injector_error_handling(
        self, mock_shim_registry, mock_compatibility_matrix
    ):
        ...
        with pytest.raises(ValueError, match="No compatible adapters provided"):
            request = ConversionRequest(
                None, DataFormat.PANDAS_DATAFRAME, DataFormat.NUMPY_ARRAY
            )
...
class TestPerformanceAndCaching:
    ...
    def test_analysis_caching_performance(
        ...
        result1 = analyzer.analyze_pipeline(
            sample_pipeline_steps, pipeline_id="perf_test"
        )
    ...
    def test_cache_size_management(
        ...
        for i in range(105):  # More than the 100 limit
            analyzer.analyze_pipeline(
                sample_pipeline_steps, pipeline_id=f"cache_test_{i}"
            )
...
class TestComplexPipelineScenarios:
    ...
    def test_multi_domain_pipeline_analysis(
        self, mock_compatibility_matrix, mock_shim_registry
    ):
    ...
    def test_pipeline_with_shim_injection_and_reanalysis(
        self, mock_compatibility_matrix, mock_shim_registry
    ):
        ...
        result = validator.validate_and_fix_pipeline(
            pipeline_steps=pipeline_steps, auto_fix=True, validation_level="strict"
        )
    ...
    def test_circular_dependency_detection(
        ...
        validator = PipelineValidator(
            mock_compatibility_matrix, mock_shim_registry, Mock(), Mock()
        )
