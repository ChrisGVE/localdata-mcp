...
class TestTimeSeriesIntegration:
    ...
    def create_test_databases(self):
        ...
        with sqlite3.connect(multi_db_path) as conn:
            self.multivariate_ts.to_sql(
                "multi_data", conn, index=True, if_exists="replace"
            )
        ...
        with sqlite3.connect(cp_db_path) as conn:
            self.changepoint_ts.to_sql(
                "changepoint_data", conn, index=True, if_exists="replace"
            )
...
class TestIndividualToolIntegration(TestTimeSeriesIntegration):
    ...
    def test_exponential_smoothing_integration(self):
        ...
        for method in methods:
            ...
            print(
                f"✅ {method} exponential smoothing completed in {execution_time:.2f}s"
            )
    ...
    def test_anomaly_detection_integration(self):
        ...
        for method in methods:
            ...
            print(
                f"✅ {method} anomaly detection completed in {execution_time:.2f}s, found {anomaly_count} anomalies"
            )
    ...
    def test_change_point_detection_integration(self):
        ...
        for method in methods:
            ...
            print(
                f"✅ {method} change point detection completed in {execution_time:.2f}s, found {change_point_count} change points"
            )
    ...
    def test_multivariate_analysis_integration(self):
        ...
        print(
            f"✅ Granger causality analysis completed in {granger_execution_time:.2f}s"
        )
...
class TestCrossComponentWorkflows(TestTimeSeriesIntegration):
    ...
    def test_comprehensive_time_series_pipeline(self):
        ...
        print(
            f"✅ Complete time series pipeline completed in {total_execution_time:.2f}s"
        )
...
class TestPerformanceBenchmarking(TestTimeSeriesIntegration):
    ...
    def test_high_frequency_data_performance(self):
        ...
        assert (
            basic_time < 45.0
        )  # Should complete within 45 seconds for high-frequency data
    ...
    def test_memory_usage_validation(self):
        ...
        for operation_name, operation_func in operations:
            ...
            # Update peak memory
            memory_metrics["peak_memory_mb"] = max(
                memory_metrics["peak_memory_mb"], after_memory
            )

            # Validate operation completed successfully
            result_data = json.loads(result)
            assert (
                "analysis_type" in result_data
                or "detection_method" in result_data
                or "model_type" in result_data
            )

            print(
                f"   Memory: {before_memory:.1f}MB → {after_memory:.1f}MB (Δ{after_memory - before_memory:+.1f}MB)"
            )
        ...
        memory_increase = (
            memory_metrics["peak_memory_mb"] - memory_metrics["initial_memory_mb"]
        )
        assert memory_increase < 1000, (
            f"Memory usage increased by {memory_increase:.1f}MB (limit: 1000MB)"
        )
    ...
    def test_streaming_architecture_validation(self):
        ...
        assert status_time < 0.1, (
            f"Tool discovery took {status_time:.3f}s (limit: 0.1s)"
        )
        ...
        assert processing_time < 30.0, (
            f"Streaming processing took {processing_time:.2f}s (limit: 30s)"
        )
...
class TestCrossDomainIntegration(TestTimeSeriesIntegration):
    ...
    def test_time_series_with_pattern_recognition(self):
        ...
        general_anomalies = len(
            cross_domain_results["anomaly_detection"]["anomaly_labels"]
        )
        ts_anomalies = len(
            cross_domain_results["ts_anomaly_detection"]["anomaly_indices"]
        )

        print(f"📋 Cross-Domain Results Comparison:")
        print(
            f"   VAR Model Variables: {len(cross_domain_results['time_series']['variables'])}"
        )
        print(
            f"   Clustering Labels: {len(set(cross_domain_results['clustering']['labels']))}"
        )
    ...
    def test_time_series_with_statistical_analysis(self):
        ...
        for col_name, col_profile in stats_integration_results["profiling"][
            "column_profiles"
        ].items():
        ...
        if profile_stats and ts_stats:
            ...
            mean_diff = abs(ts_mean - profile_mean) / max(
                abs(ts_mean), abs(profile_mean), 1e-10
            )
            assert mean_diff < 0.01, (
                f"Mean values inconsistent: TS={ts_mean}, Profile={profile_mean}"
            )
...
class TestErrorHandlingAndEdgeCases(TestTimeSeriesIntegration):
    ...
    def test_insufficient_data_handling(self):
        ...
        conn_result = _db_manager.connect_database(
            ...
            db_type="sqlite",
            conn_string=self.db_connections[
                "changepoint"
            ],  # Use any existing DB for setup
        )
        assert json.loads(conn_result)["success"] is True
        ...
        conn_result = _db_manager.connect_database(
            name="test_very_short", db_type="sqlite", conn_string=str(short_db_path)
        )
        assert json.loads(conn_result)["success"] is True
        ...
        if "error" not in arima_data:
            ...
            assert (
                "insufficient" in arima_data["error"].lower()
                or "data" in arima_data["error"].lower()
            )
    ...
    def test_missing_data_handling(self):
        ...
        missing_data = pd.DataFrame({"date": dates, "value": values})
        ...
        conn_result = _db_manager.connect_database(
            name="test_missing", db_type="sqlite", conn_string=str(missing_db_path)
        )
        assert json.loads(conn_result)["success"] is True
