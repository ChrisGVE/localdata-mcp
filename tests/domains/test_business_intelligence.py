...
def sample_transaction_data():
    ...
    for customer in customers:
        ...
        for _ in range(num_transactions):
            ...
            transactions.append(
                {"customer_id": customer, "date": transaction_date, "amount": amount}
            )
...
def sample_ab_test_data():
    ...
    control_data = pd.DataFrame(
        {"group": ["control"] * n_control, "converted": control_conversions}
    )

    # Treatment group: 12% conversion rate
    treatment_conversions = np.random.binomial(1, 0.12, n_treatment)
    treatment_data = pd.DataFrame(
        {"group": ["treatment"] * n_treatment, "converted": treatment_conversions}
    )
...
class TestRFMAnalysisTransformer:
    ...
    def test_rfm_analysis_custom_columns(self, sample_transaction_data):
        ...
        # Rename columns
        data = sample_transaction_data.rename(
            columns={
                "customer_id": "cust_id",
...
class TestBusinessIntelligencePipeline:
    ...
    def test_customer_journey_analysis(
        self, sample_transaction_data, sample_touchpoint_data, sample_funnel_data
    ):
...
class TestEnhancedABTest:
    ...
    def test_enhanced_ab_test_basic(self, sample_ab_test_data):
        ...
        with patch(
            "src.localdata_mcp.domains.business_intelligence.run_hypothesis_test"
        ) as mock_stat:
...
class TestIntegrationScenarios:
    ...
    def test_customer_segmentation_to_targeting(self, sample_transaction_data):
        ...
        champions = rfm_result.segments[rfm_result.segments["Segment"] == "Champions"]
