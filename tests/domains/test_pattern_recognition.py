...
def sample_clustering_data():
    ...
    X, y = make_blobs(
        n_samples=300, centers=4, n_features=5, random_state=42, cluster_std=1.0
    )
    return X, y
...
class TestClusteringTransformer:
    ...
    def test_hierarchical_clustering(self, sample_clustering_data):
        ...
        clusterer = ClusteringTransformer(algorithm="hierarchical", n_clusters=4)
        clusterer.fit(X)
    ...
    def test_dbscan_clustering(self, sample_clustering_data):
        ...
        clusterer = ClusteringTransformer(algorithm="dbscan", eps=1.0, min_samples=5)
        clusterer.fit(X)
    ...
    def test_gmm_clustering(self, sample_clustering_data):
        ...
        clusterer = ClusteringTransformer(
            algorithm="gmm", n_clusters=4, random_state=42
        )
        clusterer.fit(X)
    ...
    def test_spectral_clustering(self, sample_clustering_data):
        ...
        clusterer = ClusteringTransformer(
            algorithm="spectral", n_clusters=4, random_state=42
        )
        clusterer.fit(X)
    ...
    def test_auto_k_selection(self, sample_clustering_data):
        ...
        clusterer = ClusteringTransformer(
            algorithm="kmeans", auto_k_selection=True, k_range=(2, 8), random_state=42
        )
        clusterer.fit(X)
    ...
    def test_clustering_edge_cases(self):
        ...
        clusterer = ClusteringTransformer(
            algorithm="kmeans", n_clusters=2, random_state=42
        )
...
class TestDimensionalityReductionTransformer:
    ...
    def test_pca_reduction(self, sample_classification_data):
        ...
        reducer = DimensionalityReductionTransformer(
            algorithm="pca", n_components=5, random_state=42
        )
        reducer.fit(X)
    ...
    def test_tsne_reduction(self, sample_classification_data):
        ...
        reducer = DimensionalityReductionTransformer(
            algorithm="tsne", n_components=2, random_state=42
        )
        reducer.fit(X)
    ...
    def test_umap_reduction(self, sample_classification_data):
        ...
        reducer = DimensionalityReductionTransformer(
            algorithm="umap", n_components=3, random_state=42
        )
        reducer.fit(X)
    ...
    def test_ica_reduction(self, sample_classification_data):
        ...
        reducer = DimensionalityReductionTransformer(
            algorithm="ica", n_components=4, random_state=42
        )
        reducer.fit(X)
    ...
    def test_lda_reduction(self, sample_classification_data):
        ...
        reducer = DimensionalityReductionTransformer(
            algorithm="lda", n_components=2, random_state=42
        )
        reducer.fit(X, y)
    ...
    def test_auto_component_selection_pca(self, sample_classification_data):
        ...
        reducer = DimensionalityReductionTransformer(
            algorithm="pca", preserve_variance=0.9, random_state=42
        )
        reducer.fit(X)
    ...
    def test_reduction_transform(self, sample_classification_data):
        ...
        reducer = DimensionalityReductionTransformer(
            algorithm="pca", n_components=3, random_state=42
        )
...
class TestAnomalyDetectionTransformer:
    ...
    def test_isolation_forest_detection(self, sample_anomaly_data):
        ...
        detector = AnomalyDetectionTransformer(
            algorithm="isolation_forest", contamination=0.1, random_state=42
        )
        detector.fit(X)
    ...
    def test_one_class_svm_detection(self, sample_anomaly_data):
        ...
        detector = AnomalyDetectionTransformer(algorithm="one_class_svm", nu=0.1)
        detector.fit(X)
    ...
    def test_lof_detection(self, sample_anomaly_data):
        ...
        detector = AnomalyDetectionTransformer(
            algorithm="lof", contamination=0.1, n_neighbors=20
        )
        detector.fit(X)
    ...
    def test_statistical_zscore_detection(self, sample_anomaly_data):
        ...
        detector = AnomalyDetectionTransformer(
            algorithm="statistical", method="zscore", threshold_std=3.0
        )
        detector.fit(X)
    ...
    def test_statistical_iqr_detection(self, sample_anomaly_data):
        ...
        detector = AnomalyDetectionTransformer(algorithm="statistical", method="iqr")
        detector.fit(X)
    ...
    def test_anomaly_edge_cases(self):
        ...
        detector = AnomalyDetectionTransformer(
            algorithm="isolation_forest", contamination=0.05, random_state=42
        )
...
class TestHighLevelFunctions:
    ...
    def test_detect_anomalies_function(self, sample_anomaly_data):
        ...
        result = detect_anomalies(
            X, algorithm="isolation_forest", contamination=0.1, y_true=y_true
        )
    ...
    def test_evaluate_patterns_function(self, sample_clustering_data):
        ...
        clustering_result = perform_clustering(
            X, algorithm="kmeans", n_clusters=4, y_true=y_true
        )

        # Then evaluate them
        evaluation_result = evaluate_patterns(
            X, "clustering", clustering_result, y_true
        )
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_missing_dependencies(self):
        ...
        with patch(
            "src.localdata_mcp.domains.pattern_recognition.UMAP_AVAILABLE", False
        ):
...
class TestResultDataclasses:
    ...
    def test_dimensionality_reduction_result_serialization(
