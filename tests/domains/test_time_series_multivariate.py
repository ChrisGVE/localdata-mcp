...
def sample_causal_data():
    ...
    return pd.DataFrame(
        {"cause": cause, "effect": effect, "independent": independent}, index=dates
    )
...
class TestMultivariateTimeSeriesTransformer:
    ...
    def test_init(self):
        ...
        transformer = MultivariateTimeSeriesTransformer(
            min_series=3, max_series=5, require_stationarity=True
        )
    ...
    def test_validate_multivariate_data_success(self, sample_multivariate_data):
        ...
        transformer = MultivariateTimeSeriesTransformer()
        validated_data = transformer._validate_multivariate_data(
    ...
    def test_validate_multivariate_data_insufficient_series(
        ...
        with pytest.raises(
            TimeSeriesValidationError, match="Insufficient number of time series"
        ):
    ...
    def test_check_multicollinearity_warning(self):
        ...
        data = pd.DataFrame({"series1": series1, "series2": series2}, index=dates)
...
class TestVARModelForecaster:
    ...
    def test_analysis_logic_success(self, sample_multivariate_data):
        ...
        forecaster = VARModelForecaster(
            max_lags=3, forecast_horizon=5, confidence_level=0.90
        )
    ...
    def test_analysis_logic_with_small_dataset(self):
        ...
        small_data = pd.DataFrame(
            {"series1": np.random.randn(15), "series2": np.random.randn(15)},
            index=dates,
        )
...
class TestGrangerCausalityAnalyzer:
    ...
    def test_generate_granger_interpretation(self):
        ...
        significant_relationships = [
            {"relationship": "X → Y", "p_value": 0.001, "lag": 2}
        ]
...
class TestErrorHandling:
    ...
    def test_insufficient_data_handling(self):
        ...
        insufficient_data = pd.DataFrame(
            {"series1": np.random.randn(10), "series2": np.random.randn(10)},
            index=dates,
        )
    ...
    def test_single_series_validation_error(self):
        ...
        dates = pd.date_range("2020-01-01", periods=100, freq="D")
        single_series = pd.DataFrame({"series1": np.random.randn(100)}, index=dates)
