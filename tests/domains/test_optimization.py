...
class TestNetworkAnalyzer:
    ...
    def test_initialization(self):
        ...
        analyzer = NetworkAnalyzer(algorithm="dijkstra", directed=True, weighted=True)
    ...
    def test_networkx_not_available_error(self):
        ...
        with patch("localdata_mcp.domains.optimization.NETWORKX_AVAILABLE", False):
    ...
    def test_fit_from_adjacency_matrix(self):
        ...
        adj_matrix = np.array([[0, 1, 2], [1, 0, 1], [2, 1, 0]])
        ...
        assert (
            analyzer.graph_.number_of_edges() == 3
        )  # Undirected, so each edge counted once
    ...
    def test_tsp_solution(self):
        ...
        adj_matrix = np.array([[0, 1, 2, 3], [1, 0, 4, 2], [2, 4, 0, 1], [3, 2, 1, 0]])
    ...
    def test_minimum_spanning_tree(self):
        ...
        adj_matrix = np.array([[0, 4, 2, 3], [4, 0, 1, 5], [2, 1, 0, 6], [3, 5, 6, 0]])
...
class TestAssignmentSolver:
    ...
    def test_initialization(self):
        ...
        solver = AssignmentSolver(
            method="linear_programming", maximize=True, allow_partial=True
        )
    ...
    def test_maximization_assignment(self):
        ...
        profit_matrix = np.array([[1, 4, 2], [3, 2, 0], [2, 1, 3]])
    ...
    def test_greedy_assignment(self):
        ...
        cost_matrix = np.array([[1, 9, 9], [9, 1, 9], [9, 9, 1]])
    ...
    def test_linear_programming_assignment(self):
        ...
        cost_matrix = np.array([[2, 3, 1], [1, 4, 2], [3, 1, 3]])
    ...
    def test_assignment_matrix_format(self):
        ...
        if result.success:
            ...
            assert np.all(
                (result.assignment_matrix == 0) | (result.assignment_matrix == 1)
            )
...
class TestOptimizationTools:
    ...
    def test_analyze_network_tool(self, mock_db_manager):
        ...
        with patch("pandas.read_sql") as mock_read_sql:
            mock_read_sql.return_value = pd.DataFrame(
                {"source": [0, 1, 0], "target": [1, 2, 2], "weight": [1.0, 2.0, 1.5]}
            )
