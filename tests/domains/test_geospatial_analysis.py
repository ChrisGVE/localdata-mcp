...
pytestmark = pytest.mark.skipif(
    not _geospatial_available(), reason="geopandas not installed"
)
...
class TestSpatialTransformers:
    ...
    def test_distance_transformer(self):
        ...
        data = pd.DataFrame({"x": [0, 1, 2], "y": [0, 1, 2], "value": [10, 20, 30]})

        transformer = SpatialDistanceTransformer(
            coordinate_columns=["x", "y"], distance_metrics=["euclidean"]
        )
    ...
    def test_geometry_transformer(self):
        ...
        data = pd.DataFrame(
            {"x": [0, 1, 2, 0], "y": [0, 0, 1, 1], "value": [10, 20, 30, 40]}
        )

        transformer = SpatialGeometryTransformer(
            coordinate_columns=["x", "y"], operations=["bounding_box", "convex_hull"]
        )
...
class TestSpatialJoinEngine:
    ...
    def test_spatial_join_basic(self):
        ...
        left_data = pd.DataFrame(
            {"geometry": [(0, 0), (1, 1), (2, 2)], "left_value": [10, 20, 30]}
        )
...
class TestHighLevelFunctions:
    ...
    def test_analyze_spatial_autocorrelation(self):
        ...
        result = analyze_spatial_autocorrelation(
            data, value_column="value", coordinate_columns=["x", "y"], method="moran"
        )
    ...
    def test_perform_spatial_clustering(self):
        ...
        result = perform_spatial_clustering(
            data, coordinate_columns=["x", "y"], method="hotspot"
        )
    ...
    def test_calculate_spatial_distance(self):
        ...
        data1 = pd.DataFrame({"x": [0, 1, 2], "y": [0, 0, 0]})

        data2 = pd.DataFrame({"x": [0, 1], "y": [1, 1]})
...
class TestGeospatialAnalysisPipeline:
    ...
    def test_pipeline_initialization(self):
        ...
        pipeline = GeospatialAnalysisPipeline(
            coordinate_columns=["x", "y"], value_column="value"
        )
    ...
    def test_pipeline_execution(self):
        ...
        data = pd.DataFrame(
            {"x": [0, 1, 2, 3, 4], "y": [0, 1, 2, 3, 4], "value": [10, 20, 30, 40, 50]}
        )
