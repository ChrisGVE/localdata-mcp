...
def _patch_security_for_tempdir(temp_dir):
    """Create a patch that allows the given temp directory in path security."""
    mock_sec_cfg = MagicMock()
    mock_sec_cfg.restrict_paths = True
    mock_sec_cfg.allowed_paths = [".", temp_dir]
...
class TestMultiSheetHandling:
    ...
    def _allow_temp_paths(self):
        """Patch config manager to allow temp directory paths."""
        temp_dir = tempfile.gettempdir()
        with _patch_security_for_tempdir(temp_dir):
            yield
    ...
    def test_excel_multi_sheet_connection(self, manager, multi_sheet_data):
        ...
        try:
            ...
            emp_result = manager.execute_query(
                "multi_excel", "SELECT COUNT(*) FROM employees"
            )
            assert "3" in emp_result

            prod_result = manager.execute_query(
                "multi_excel", "SELECT COUNT(*) FROM products"
            )
            assert "3" in prod_result

            sales_result = manager.execute_query(
                "multi_excel", "SELECT COUNT(*) FROM sales"
            )
            assert "3" in sales_result

            # Test cross-sheet queries (JOIN across sheets)
            join_result = manager.execute_query(
                "multi_excel", "SELECT e.name, e.department FROM employees e"
            )
    ...
    def test_excel_specific_sheet_connection(self, manager, multi_sheet_data):
        ...
        try:
            ...
            result = manager.connect_database(
                "single_sheet", "excel", temp_path, sheet_name="employees"
            )
            assert "Successfully connected" in result
    ...
    def test_ods_multi_sheet_connection(self, manager, multi_sheet_data):
        ...
        try:
            ...
            result = manager.execute_query(
                "multi_ods",
                "SELECT product_name FROM products WHERE category = 'Widgets'",
            )
    ...
    def test_sheet_name_sanitization_edge_cases(self, manager):
        ...
        test_cases = [
            ...
            (
                "Very_Long_Sheet_Name_That_Should_Still_Work_Fine",
                "Very_Long_Sheet_Name_That_Should_Still_Work_Fine",
            ),
        ]
        ...
        for original, expected_start in test_cases:
            ...
            assert (
                result.startswith(expected_start.split("_")[0])
                or result == expected_start
            )
    ...
    def test_empty_sheets_handling(self, manager):
        ...
        try:
            ...
            with pd.ExcelWriter(temp_path, engine="openpyxl") as writer:
                # Non-empty sheet
                pd.DataFrame({"col1": [1, 2, 3]}).to_excel(
                ...
                pd.DataFrame({"col2": ["a", "b"]}).to_excel(
                    writer, sheet_name="text_sheet", index=False
                )

            # Connect and test - may fail if streaming processor can't handle empty sheets
            ...
            if "Successfully connected" in result:
                ...
                data_result = manager.execute_query(
                    "mixed_sheets", "SELECT COUNT(*) FROM data_sheet"
                )
                assert "3" in data_result

                text_result = manager.execute_query(
                    "mixed_sheets", "SELECT COUNT(*) FROM text_sheet"
                )
                assert "2" in text_result

                manager.disconnect_database("mixed_sheets")
            else:
                # Known issue: empty sheets can cause streaming processor errors
    ...
    def test_sheet_with_special_characters(self, manager):
        ...
        special_data = pd.DataFrame({"test_col": ["value1", "value2", "value3"]})
        ...
        try:
            ...
            with pd.ExcelWriter(temp_path, engine="openpyxl") as writer:
                special_data.to_excel(
                    writer, sheet_name="Sheet With Spaces", index=False
                )
                special_data.to_excel(
                    writer, sheet_name="Sheet-With-Hyphens", index=False
                )
            ...
            assert (
                "Sheet_With_Spaces" in description or "Sheet_with_spaces" in description
            )
            assert (
                "Sheet_With_Hyphens" in description
                or "Sheet_with_hyphens" in description
            )

            # Test querying tables with sanitized names
            tables_result = manager.execute_query(
                "special_chars", "SELECT name FROM sqlite_master WHERE type='table'"
            )
            ...
            if table_names:
                ...
                query_result = manager.execute_query(
                    "special_chars", f"SELECT COUNT(*) FROM {table_name}"
                )
    ...
    def test_nonexistent_sheet_name(self, manager, multi_sheet_data):
        ...
        try:
            ...
            result = manager.connect_database(
                "nonexistent", "excel", temp_path, sheet_name="DoesNotExist"
            )
            assert (
    ...
    def test_large_multi_sheet_file(self, manager):
        ...
        try:
            ...
            result1 = manager.execute_query(
                "many_sheets", "SELECT COUNT(*) FROM Sheet_0"
            )
            assert "10" in result1

            result2 = manager.execute_query(
                "many_sheets", "SELECT COUNT(*) FROM Sheet_5"
            )
...
class TestMultiSheetIntegration:
    ...
    def test_get_table_sample_multi_sheet(self, manager):
        ...
        try:
            ...
            # Test getting sample data via query (get_table_sample not available)
            sample = manager.execute_query("store", "SELECT * FROM inventory LIMIT 3")
