...
class TestSecurityIntegration:
    ...
    def test_complete_security_pipeline_valid_query(
        ...
        with (
            ...
            mock_conn_mgr.return_value.managed_query_execution.return_value.__enter__ = Mock(
                return_value={"start_time": time.time(), "query_id": "test_123"}
            )
            mock_conn_mgr.return_value.managed_query_execution.return_value.__exit__ = (
    ...
    def test_complete_security_pipeline_basic_sql_violation(
        self, enhanced_tools, mock_database_config
    ):
...
class TestSecurityManagerLifecycle:
    ...
    def test_security_manager_resource_cleanup(self):
        ...
        with (
            ...
            manager.validate_query_security(
                "SELECT id FROM users", "test_db", "test_conn"
            )
