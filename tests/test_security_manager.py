...
class TestSecurityManager:
    ...
    def test_comprehensive_security_validation_complex_query(self, security_manager):
        ...
        query = (
            "SELECT * FROM "
            + " INNER JOIN ".join([f"table{i}" for i in range(10)])
            + " WHERE 1=1"
        )
        database_name = "test_db"
        connection_id = "test_connection"
    ...
    def test_secure_query_execution_context(self, security_manager):
        ...
        with (
            ...
            with security_manager.secure_query_execution(
                query, database_name, connection_id
            ) as context:
    ...
    def test_security_event_logging(self, security_manager):
        ...
        injection_events = [
            e for e in events if e.event_type == SecurityEventType.INJECTION_ATTEMPT
        ]
    ...
    def test_security_event_filtering(self, security_manager):
        ...
        assert all(
            e.event_type == SecurityEventType.INJECTION_ATTEMPT
        ...
        assert all(
            e.threat_level == SecurityThreatLevel.CRITICAL for e in critical_events
        )
    ...
    def test_security_statistics(self, security_manager):
        ...
        # Check configuration
        assert (
            stats["configuration"]["audit_enabled"]
            == security_manager.config.audit_enabled
        )
    ...
    def test_comprehensive_owasp_validation(self, security_manager):
        ...
        owasp_test_cases = [
            ...
            (
                ...
                [AttackPattern.UNION_INJECTION, AttackPattern.COMMENT_INJECTION],
            ),
            # Time-based blind injection
            (
                "1' AND sleep(5)--",
                [AttackPattern.TIME_BASED_BLIND, AttackPattern.COMMENT_INJECTION],
            ),
            # Information schema exploitation
            (
                "' UNION SELECT table_name FROM information_schema.tables--",
                [
                    AttackPattern.UNION_INJECTION,
                    AttackPattern.INFORMATION_EXTRACTION,
                    AttackPattern.COMMENT_INJECTION,
                ],
            ),
            # Stacked queries with data manipulation
            (
                "1'; DROP TABLE users; --",
                [AttackPattern.STACKED_QUERIES, AttackPattern.COMMENT_INJECTION],
            ),
            # File system access
            (
        ...
        with (
            ...
            for malicious_query, expected_patterns in owasp_test_cases:
                ...
                assert metadata["threat_level"] in [
                ...
                detected_patterns = security_manager.detect_attack_patterns(
                    malicious_query
                )
