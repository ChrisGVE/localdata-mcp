...
class TestBackwardCompatibilityIntegration(unittest.TestCase):
    ...
    def test_complete_legacy_detection_workflow(self):
        ...
        with patch.dict(os.environ, self.test_env, clear=True):
            ...
            self.assertIn(
                "single_database_env_vars", self.manager._deprecation_warnings_shown
            )
    ...
    def test_api_backward_compatibility_scenarios(self):
        ...
        compat_info = self.manager.check_api_compatibility(
            "execute_query", ("my_database", "SELECT * FROM users"), {}
        )
        ...
        self.assertTrue(compat_info["compatible"])
        self.assertEqual(
            len(compat_info["warnings"]), 0
        )  # No warnings for modern usage
    ...
    def test_response_format_migration(self):
        ...
        new_response = {
            ...
            "metadata": {
                "rows": 2,
                "columns": 2,
                "query_analysis": {"estimated_time": "10ms"},
            },
        }
        ...
        migrated = self.manager.migrate_legacy_response_format(
            new_response_result, "v1.3.0"
        )
    ...
    def test_compatibility_status_reporting(self):
        ...
        with patch.dict(os.environ, self.test_env, clear=True):
            ...
            self.assertIn(
                "single_database_env_vars", status["deprecation_warnings_shown"]
            )
